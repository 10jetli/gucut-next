 ...
 * 🔒 **ไม่แตะไฟล์บิลเลย: ไม่ลบ ไม่ย้าย ไม่เปลี่ยนชื่อ ไม่อัปทับ** — การลบบิลต้องให้คนตัดสินทีละใบ
 *    (บิลคือเอกสารบัญชี · ลบแล้วไม่มีถังขยะให้กู้ — กฎในหัวไฟล์ lib/billblobs.ts)
 *
 * ✍️ **แต่ตามค่าตั้งต้น เส้นนี้ "เขียน" หนึ่งอย่าง: แคชรอบบิล** (คีย์ `p/<vendor>` ใน Blobs)
 *    คือแผนที่ "ชื่อไฟล์ → รอบบิลที่อ่านได้จากเนื้อใน" ซึ่งจอบิลใช้จัดเดือน
 *    เหตุที่ให้เส้นนี้เติม: มันเป็นเส้นเดียวที่เปิดอ่าน PDF ได้โดยไม่ต้องล็อกอิน
 *
 *    🔴 **แต่ตัวตรวจที่เขียนของที่มันตรวจ = ผลเชื่อไม่ได้** (ฝั่ง CTO ทักเอง 18 ก.ย. 2569)
 *       ยิงรอบแรกได้ค่าหนึ่ง ยิงรอบสองได้อีกค่า แล้วแยกไม่ออกว่าเปลี่ยนเพราะระบบ
 *       หรือเปลี่ยนเพราะการไปยิงเอง ⇒ รายงานให้ท่านประธานไม่ได้ทั้งใบ
 *    ⇒ **`nowrite=1` ปิดการเขียนทั้งหมด** และคำตอบมีช่อง `เขียนแคชรอบนี้` บอกตรง ๆ ทุกรอบ
 *       ว่าเขียนหรือไม่เขียน · ใช้ `nowrite=1` ทุกครั้งที่ยิงเพื่อ **ยืนยันตัวเลข**
...
export async function GET(req: NextRequest) {
  ...
  /* 🔴 nowrite=1 ⇒ **ห้ามเขียนแคชรอบบิล** — สำหรับรอบที่ใช้ "ยืนยันตัวเลข"
     ตัวตรวจต้องไม่เปลี่ยนสถานะของสิ่งที่มันตรวจ (ดูเหตุผลเต็มบนหัวไฟล์) */
  const nowrite = sp.get('nowrite') === '1'
  ...
  for (const vendorId of vendors) {
    ...
    /* เขียนแคชที่อ่านได้รอบนี้กลับไป — จอบิลจะจัดเดือนถูกทันทีโดยไม่ต้องแกะ PDF เอง
       ⚠️ nowrite=1 ⇒ ไม่เขียน และต้อง **บอกในคำตอบ** ว่ารอบนี้ไม่ได้เขียน ไม่ใช่เงียบ */
    ...
    out.push({
      ...
      /* ✍️ บอกทุกรอบว่ารอบนี้เขียนแคชหรือไม่ — ตัวตรวจต้องรู้ว่าตัวเองเปลี่ยนอะไรไปหรือเปล่า */
      เขียนแคชรอบนี้,
      ...(nowrite ? { nowrite: true } : {}),
  ...
  return NextResponse.json({
    ok: true,
    หมายเหตุ: 'ไม่แตะไฟล์บิลเลย (ไม่ลบ ไม่ย้าย ไม่เปลี่ยนชื่อ) · '
      + (nowrite
        ? 'รอบนี้ nowrite=1 ⇒ **ไม่เขียนอะไรเลย** แม้แต่แคชรอบบิล'
        : 'แต่รอบนี้ **เขียนแคชรอบบิล** (แผนที่ชื่อไฟล์→รอบบิล ที่จอบิลใช้จัดเดือน) — '
          + 'ถ้ากำลังยืนยันตัวเลข ให้ใส่ nowrite=1 เพื่อไม่ให้ตัวตรวจเปลี่ยนสิ่งที่มันตรวจ')
      + ' · ตัวเลข "จำนวนใบซ้ำ" = ไฟล์ที่เกินมาจากใบเดียวกัน',
