...
export async function GET(request: Request) {
  ...
  try {
    ...
    /* 🔴 **เดิมเขียนแคชแบบไม่รอ — บน Netlify แปลว่าแคชไม่เคยถูกเขียนเลย** (แก้ 19 ก.ย. 2569)
       คอมเมนต์เดิมว่า "ผู้ใช้ไม่ควรต้องรอขั้นตอนนี้" ฟังดูถูก แต่ Netlify **แช่แข็งฟังก์ชัน
       ทันทีที่ตอบคำขอเสร็จ** ⇒ งานที่ยิงทิ้งไว้ตายกลางทางแบบไม่มี error ให้เห็น
       ⇒ อาการที่ได้คือ "แคชที่ไม่เคยเข้า" — คำขอถัดไปคิดใหม่ทุกครั้ง **โดยไม่มีอะไรฟ้อง**
          และยิ่งแพงกว่าการรอเขียน เพราะ computeReorder หนักกว่าการเขียน Blobs มาก
       📌 ฝั่งท่อโดนคลาสนี้มาแล้ว 3 ครั้ง (รูปสแกนไม่ถูกเก็บ · แคชสต็อกค้าง 5 วัน)
          ⇒ ด่าน check-floating ที่ย้ายมาจากฝั่งท่อวันนี้จับจุดนี้ได้ในการรันครั้งแรก */
