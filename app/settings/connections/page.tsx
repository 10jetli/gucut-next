...
interface Conn {
  ...
  timedOut?: boolean
  /* ── วันหมดอายุ token (UTC) — ขอไว้ 4 ก.ย. 2569 · **ท่อส่งมาแล้ว** (ยิงเช็ค 17 ก.ย. 2569: ทุกแถวมาร์เก็ตเพลสมีช่องนี้)
     ⚠️ อย่าสับกับ `tokenRefresh.result[*].hoursLeft` — นั่นคือภาพของ **รอบต่ออายุเมื่อคืน** แต่ชั่วโมงคิดเทียบ "ตอนนี้"
        ⇒ Shopee ขึ้น expired:true · hoursLeft -13 ทั้งที่ token ปัจจุบันยังเหลือ 3 ชม. · ใช้ช่องรายแถวข้างล่างเท่านั้น ──
  ...
  return (
    <div className="p-4 md:p-6">
      ...
      {loading && !data && (
        <>
          <LoadingState />
          {/* 🔴 วัดจริง 6 ก.ย. 2569: เปิดครั้งแรกของวัน **14.75 วินาที** · ครั้งถัด ๆ ไป 0.8–1.7 วิ
              (ยิงซ้ำ 3 รอบติดกันได้ 1.70 / 1.01 / 0.81) ⇒ ตัวเลขแรกคือ **ตอนเย็น ไม่ใช่ค่าปกติ**
              ⚠️ ถ้าวัดรอบเดียวแล้วรายงาน จะได้ข้อสรุปผิดทั้งสองทาง —
                 วัดรอบแรกอย่างเดียว = "จอนี้ช้า 15 วิ" · วัดรอบหลังอย่างเดียว = "เร็ว 1 วิ"
              ⇒ จอต้องบอกคนใช้ว่ารอนานได้ ไม่งั้นเขาจะคิดว่าจอค้างแล้วกดรีเฟรชซ้ำ
                 (ซึ่งยิ่งช้าเพราะเริ่มยิงใหม่ทั้งชุด) */}
          <p className="text-[12.5px] text-gray-500 text-center mt-2 leading-relaxed">
            กำลัง<b>ยิงของจริง</b>ไปหาทุกเจ้า (ZORT · Shopee · Lazada · TikTok · LINE ฯลฯ) —
            <b> เปิดครั้งแรกของวันใช้เวลาได้ถึง ~15 วินาที</b> ครั้งถัดไปไม่กี่วินาที
            <br />
            <span className="text-gray-400">ยังไม่ค้าง — อย่าเพิ่งกดรีเฟรช การกดใหม่จะเริ่มยิงทั้งชุดอีกรอบ</span>
          </p>
        </>
      )}
      ...
      {/* 🔴 **จอนี้กับจอ "การเชื่อมต่อ" ของ ZORT ชื่อเดียวกัน หัวจอเหมือนกัน แต่ตอบคนละคำถาม**
          เทียบจอจริงทีละแถว 17 ก.ย. 2569 (ใบ t_mu2u9mym · อ่านอย่างเดียว):
          · ZORT ตอบว่า **มีทะเบียนการเชื่อมต่ออะไรตั้งไว้ในบัญชี ZORT บ้าง** (รวมกุญแจ API ซึ่งจอนี้ไม่แสดง)
          · จอนี้ตอบว่า **ยิงของจริงแล้วเจ้าไหนตอบ** (รวมหน้าร้านของเราเองและบัญชี ZORT ซึ่ง ZORT ไม่ได้นับ)
          ⚠️ วันที่เทียบ **เลขบนหัวจอสองฝั่งบังเอิญเท่ากันพอดี** ทั้งที่เป็นคนละชุด
             ⇒ ตรงกับกฎใน CLAUDE.md ข้อ 4: ตัวเลขสองแหล่งวางคู่กันโดยไม่บอกว่าต่างกันตรงไหน
                คนอ่านจะเชื่อว่าเป็นชุดเดียวกันเสมอ — และที่นี่มันเท่ากันพอดีจนไม่มีอะไรสะดุดตาเลย */}
      {data && !data.skip && (
        <p className="text-[12.5px] mb-3 bg-amber-50 border border-amber-200 text-amber-900 rounded-lg px-3 py-2 leading-relaxed">
          ℹ️ <b>เลขบนหัวจอนี้ไม่ใช่เลขเดียวกับ “จำนวน N รายการ” ในจอการเชื่อมต่อของ ZORT</b> — คนละชุดกัน
          <br />· จอนี้นับเจ้าที่ระบบเรา<b>ยิงตรวจได้จริง</b> รวมหน้าร้านของเราเองและบัญชี ZORT ซึ่ง ZORT ไม่ได้นับ
          <br />· ZORT นับ<b>ทะเบียนการเชื่อมต่อในบัญชี ZORT</b> รวม<b>กุญแจ API</b> ซึ่งจอนี้ไม่แสดง
          {/* 🔬 ยืนยันด้วยตา 18 ก.ย. 2569 (อ่านอย่างเดียว · `/Integration/Main`): จอเขาขึ้น "จำนวน 10 รายการ"
              แยกได้เป็น มาร์เก็ตเพลส 3 (TIKTOK · Shopee-gucut · Lazada-gucut) · เว็บไซต์ 1 (Shopify)
              · โซเชียล 2 (เพจเฟซบุ๊ก · LINE @gucut1) · บัญชี 1 (PEAK) · **กุญแจ ZORT API 3 ใบ**
              ⇒ 3+1+2+1+3 = 10 พอดี ⇒ คำอธิบายข้างบน **ยังจริง** และตัวเลขที่ต่างกันคือกุญแจ API สามใบนั้นเอง */}
          <br />· <span className="text-amber-800">ตรวจซ้ำ 18 ก.ย. 2569: ของเขา 10 = มาร์เก็ตเพลส 3 · เว็บไซต์ 1 · โซเชียล 2 · บัญชี 1 · <b>กุญแจ API 3</b></span>
          <br />⇒ เจ้าที่ ZORT ขึ้นว่าเชื่อมอยู่ อาจขึ้นที่นี่ว่า<b>ยังไม่ได้เชื่อม</b>ได้ ถ้าเรายังยิงตรวจไม่ผ่าน —
          <b> สองอย่างนี้ไม่ขัดกัน</b> แต่ต้องอ่านให้ถูกว่าอันไหนตอบอะไร
        </p>
      )}
