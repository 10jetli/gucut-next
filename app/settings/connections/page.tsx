...
function TokenExpiry({ at, expired }: { at?: string | null; expired?: boolean | null }) {
  ...
  /* 🔴 **เงื่อนไขของคำเตือนต้องเห็นด้วยตา ห้ามซ่อนใน tooltip** (แก้ 5 ก.ย. 2569 กลางคืน)
     ของเดิมข้อความที่ **เห็นจริง** มีแค่ "🔑 token หมดอายุแล้ว" เป็นสีแดง
     ส่วนเงื่อนไข ("ตัวต่ออายุวิ่งตี 3 ครึ่งทุกวัน ⇒ หมดอายุแล้วไม่ได้แปลว่าพัง")
     อยู่ใน `title` ซึ่ง **บนมือถือไม่มีทางเห็นเลย** และบนคอมต้องชี้ค้างถึงเห็น
     ⇒ คนเปิดจอเห็นแดงว่า "หมดอายุแล้ว" จะสรุปว่าการเชื่อมต่อพัง ทั้งที่ระบบปกติดี

     ของจริงที่วัดได้คืนนี้: token Shopee บอกหมดอายุ 12:34 UTC
     แต่ยิง `?stockcompare=1` ตอน 21:xx ได้ข้อมูลครบ 5 รอบ ⇒ **ใช้งานได้จริงตลอด**
     ⇒ "หมดอายุแล้ว" ตัวแดงนี้เป็นคำเตือนที่ทำให้ตกใจเปล่าถ้าไม่มีเงื่อนไขกำกับ

     ⚠️ นี่คือคลาสเดียวกับที่แก้ที่จอรับของเข้าคืนนี้ แต่**กลับด้าน**:
        ที่นั่นบอกว่ามีตาข่ายโดยไม่บอกเงื่อนไข ⇒ คนกล้าเกินไป
        ที่นี่บอกว่ามีปัญหาโดยไม่บอกเงื่อนไข ⇒ คนตกใจเกินไป
        **คำเตือนที่ไม่มีเงื่อนไขกำกับ ทำให้คนตัดสินใจผิดได้ทั้งสองทาง** */
  return (
    <span
    ...
    >
      ...
      {gone && (
        <span className="block text-gray-500 font-normal">
          ตัวต่ออายุวิ่งทุกวันตี 3 ครึ่ง — <b>ถ้าการ์ดนี้ยังขึ้นว่าเชื่อมได้ แปลว่าต่ออายุแล้ว ไม่ต้องทำอะไร</b>
          {' '}ที่ต้องดูคือ &ldquo;เชื่อมไม่ได้&rdquo; คู่กับ &ldquo;หมดอายุ&rdquo; พร้อมกัน
        </span>
      )}
    </span>
  )
