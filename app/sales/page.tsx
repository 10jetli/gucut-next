...
interface Report {
  ...
  topError?: string
  /* 🔴 **ท่อส่งขอบเขตของยอดเงินมา แต่จอไม่เคยประกาศช่องนี้ไว้ ⇒ ไม่มีทางแสดง** (ยิงตรวจ 19 ก.ย. 2569)
     ⚠️ "มาตลอด" คือคำที่ผมเขียนไว้ตอนแรกจากการยิงครั้งเดียว ⇒ แก้เป็นสิ่งที่วัดได้จริง
...
function ReportKindSelect() {
  return (
    ...
    >
      ...
      {/* 🔴 ZORT มีสองตัวนี้ในจอเดียวกัน — ของเรา **ยังคำนวณไม่ได้** ⇒ คงช่องไว้ให้ผังตรง **แต่ห้ามโชว์ 0**
             🔎 **แคบลงแล้ว 18 ก.ย. 2569** (อ่านโค้ดจอ "ปรับต้นทุนสินค้า" ของ ZORT · อ่านอย่างเดียว):
                ZORT เก็บต้นทุนไว้ที่ **บรรทัดการเคลื่อนไหวสต็อก** (แก้เป็น `totalprice` ต่อบรรทัด)
                และมีเส้นคิดต้นทุนให้ชื่อ `dogetCalculateProductCostByStockMovements`
                รับ `stockmovementids` + `calculatecosttype` ⇒ **ฐานต้นทุนของเขาคือระดับการเคลื่อนไหว
                ไม่ใช่ราคาซื้อของตัวสินค้า** และมี "ชนิดการคิดต้นทุน" มากกว่าหนึ่งแบบ
             ⇒ ที่ยังขาดจริง ๆ คือ **กระจกเราไม่ได้เก็บต้นทุนรายการเคลื่อนไหว** (เก็บแต่ราคาซื้อรายสินค้า)
                ถ้าจะทำกำไรให้ตรงกับ ZORT ต้องดึงชั้นนี้เข้ากระจกก่อน — ไม่ใช่แค่เลือกสูตร
             🧪 **ทางลัดที่ลองแล้วยังใช้ไม่ได้ (18 ก.ย. 2569) — จดไว้กันคนถัดไปลองซ้ำ**
                Shopee ส่งช่อง `cost_of_goods_sold` มาในข้อมูลการเงินรายใบจริง (เห็นใน `fieldsSeen` 86 ช่อง)
                🔴 **แต่ค่าที่ส่งมา = ราคาขาย** เพราะร้านไม่ได้กรอกต้นทุนไว้ในระบบ Shopee
                   ⇒ เอามาคิดกำไรจะได้ **กำไร 0 ทุกใบ** ซึ่งดูเหมือนคำนวณได้แต่ผิดสนิท
