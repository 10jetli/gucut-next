...
function Inner() {
  ...
  /* 🛡️ ด่านกัน "ท่อส่งของผิดชั้น" — **เก็บไว้ถาวรโดยตั้งใจ ไม่ใช่ของชั่วคราว**
     เคยเกิดจริง 9 ก.ย. 2569: ท่อหยิบบรรทัดสินค้ามาเป็นหัวใบ ⇒ number = จำนวนสินค้า (3/2/1)
     และกล่องเงินเป็นราคาต่อหน่วยของบรรทัดแรก ซึ่ง**หน้าตาเหมือนยอดใบทุกประการ**
     ท่อแก้แล้ว (6ab53c6) และยืนยันด้วย scripts/check-detail-live.mjs — แต่ด่านนี้ราคาถูกมาก
     และเป็นอย่างเดียวที่จับได้ทันทีถ้าโครง ZORT เปลี่ยนอีก ⇒ ไม่ถอด
     เกณฑ์: ช่องที่มีได้เฉพาะในบรรทัดสินค้าโผล่ในรายชื่อช่องหัวใบ */
  ...
  return (
    <div className="p-4 md:p-6 max-w-[860px]">
      ...
      {error && !why && <ErrorBox title={isSkip(error) ? 'ยังทำงานส่วนนี้ต่อไม่ได้' : 'ดึงใบเสนอราคาไม่ได้'}>{error}</ErrorBox>}
      ...
      {!loading && !error && d && wrongLevel && (
        <div className="bg-red-50 border border-red-200 rounded-md px-3 py-2.5 mb-4 text-[12.5px] text-red-800">
          ...
          <p className="mt-1 text-[11.5px] text-red-700">
            ...
            และช่องเงินคือราคาต่อหน่วย ไม่ใช่ยอดของใบ · แจ้งฝั่งท่อทันที แล้วดูใบจริงที่ ZORT ไปก่อน
          </p>
        </div>
      )}
