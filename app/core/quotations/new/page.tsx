...
//
// 🚫 **แก้ใบ/ยกเลิกใบ = ยิงแล้วไม่ผ่าน** (เลื่อนสถานะ 6 ก.ย. 2569 จาก "ยังไม่เคยยิง")
//    EditQuotation กับ VoidQuotation มีเส้นจริง (405) แต่ POST แล้วได้ `Invalid ID` ทุกแบบ
//    (id ตัวเลข · ข้อความ · เลขที่ใบ · แนบ id บรรทัด) ทั้งที่เลขเดียวกัน GET ได้ปกติ
 ...
 *  (ZORT ไม่เปิด API ให้ลบใบ — ยิงตรวจ 6 ก.ย. 2569 ⇒ ใบผิดค้างอยู่ถาวรจนกว่าจะเข้าไปจัดการเอง)
 *
...
/* ✅ **เปิดแล้ว 6 ก.ย. 2569** — ผ่านเกณฑ์ที่ตั้งไว้ครบทั้งสองข้อ ไม่ใช่แค่ "ยิงผ่าน"
   ยิงจริง QT-202609003 (น็อต ฿10 × 3) แล้ว **ดึงใบกลับมาดู**:
   ราคาต่อหน่วย 10 · ยอดรวมบรรทัด 30 · ยอดหัวใบ 30 ⇒ เงินเข้าถูกช่องครบสามชั้น
   ⚠️ **เปิดเฉพาะใบเสนอราคาเท่านั้น** — ใบสั่งซื้อกับเพิ่มสินค้ายังปิดอยู่ ห้ามเปิดตาม
      (เจ้าของร้านอนุมัติทีละใบ · ใบสั่งซื้อแก้โค้ดแล้วแต่ยังไม่เคยยิงจริงสักใบ) */
  ...
  return (
    <div className="p-4 md:p-6 max-w-[900px]">
      ...
      <div className="text-[12.5px] text-red-900 bg-red-50 border border-red-300 rounded-md px-3.5 py-2.5 mb-4 leading-relaxed">
        ...
        <br />
        ⚠️ <b>&ldquo;ยิงผ่าน&rdquo; ไม่พอ</b> — ฝั่งท่อยิงจริงแล้ว (6 ก.ย. 2569) พบว่า ZORT
        ...
        🚫 <b>แก้ใบหรือยกเลิกใบที่ส่งไปแล้ว ทำไม่ได้</b> — ยิงจริงแล้ว ZORT ตอบ
        {' '}<code>Invalid ID</code> ทุกแบบ (6 ก.ย. 2569) ทั้งที่เลขเดียวกันนั้นอ่านได้ปกติ
        {' '}⇒ <b>ไม่ใช่ &ldquo;ยังไม่ได้ทดสอบ&rdquo; แต่คือทดสอบแล้วไม่ผ่าน</b>
        {' '}⇒ <b>ทำใบให้ถูกตั้งแต่แรก</b> ส่งผิดแล้วต้องเข้าไปยกเลิกในหน้าจอ ZORT เอง
      </div>
      ...
      <p className="text-[11.5px] text-gray-400 mt-3 leading-relaxed">
        {/* 🔴 ท่าที่พิสูจน์แล้วว่าประหยัดเอกสารจริง — ซ้อมดูตัวเลขก่อน ถึงยอมเสียใบจริง */}
        💡 <b>ก่อนกดส่งจริง ให้ดูยอดใน &ldquo;ทดลองส่ง&rdquo; ก่อนทุกครั้ง</b> —
        ถ้ายอดในนั้นยังไม่ถูก ส่งจริงไปก็ได้ใบผิดที่ลบไม่ได้ ·
        เสียเวลา 5 วินาที ประหยัดเอกสารจริงหนึ่งใบทุกครั้งที่เดาผิด
        <br />
        ...
        <br />
        ⚠️ <b>ไม่มีปุ่มแก้ใบ/ยกเลิกใบโดยตั้งใจ</b> — เส้นของ ZORT มีอยู่จริงแต่ยิงแล้วไม่ผ่าน
        (Invalid ID ทุกแบบ) ⇒ ใส่ปุ่มไปก็จะได้ปุ่มที่กดแล้วขึ้น error ตลอดกาล ·
        {/* 🔴 **ข้อความนี้กลายเป็นเท็จไปครึ่งหนึ่งวันนี้** (แก้ 18 ก.ย. 2569 · ใบ S4 ไล่คำกล่าวอ้างหมดอายุ)
            เดิมเขียนว่า "เพิ่มสินค้ากับใบสั่งซื้อยังไม่เปิด" — แต่ท่านประธานเปิด **เพิ่มสินค้า** และ **ใบขาย**
            ให้แล้ววันนี้ ⇒ จอนี้จะบอกคนใช้ว่าทำไม่ได้ ทั้งที่ทำได้แล้ว
            🔑 คลาส stale-state: ประโยคที่เล่า "สภาพตอนนี้" โดยไม่มีวันที่ จะกลายเป็นคำโกหกเงียบ ๆ
               ⇒ เขียนเป็นเหตุการณ์+วันที่ แทนการเล่าสภาพลอย ๆ */}
        <b> จอนี้สร้างได้เฉพาะใบเสนอราคา</b> (ขอบเขตของจอนี้เอง) — จอเขียนจริงที่เปิดแล้ว:
        {' '}ใบเสนอราคา (6 ก.ย. 2569) · <Link href="/core/stock/new" className="text-blue-600 hover:underline">เพิ่มสินค้า</Link>
        {' '}และ <Link href="/core/sales/new" className="text-blue-600 hover:underline">สร้างใบขาย</Link> (18 ก.ย. 2569)
        {' '}· <b>ใบสั่งซื้อยังไม่เปิด</b> ·
