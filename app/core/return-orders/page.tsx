...
export default function ReturnOrdersPage() {
  ...
  const ตัวกรองที่ท่อไม่ได้ใช้ = d?.applied
    ? ([
        [q.trim(), d.applied.q ?? '', 'คำค้น'],
        [from, d.applied.from ?? '', 'วันที่เริ่ม'],
        [to, d.applied.to ?? '', 'วันที่สิ้นสุด'],
        ...
        <>
          {partial && (
            <div className="text-[12.5px] text-gray-600 bg-gray-50 border border-gray-200 rounded-md px-3.5 py-2 mb-3 leading-relaxed">
              {/* 🔴 **สองบรรทัดในจอเดียวเคยพูดสวนกัน** (เจอด้วยการเปิดจอจริง 18 ก.ย. 2569 ค่ำ)
                  หัวจอโชว์ "มูลค่าทั้งหมด 723,346.14 (ไม่นับใบยกเลิก 152 ใบ)" ซึ่งเป็น**ยอดของทั้ง 693 ใบ**
                  จากกระจก · แต่บรรทัดนี้เขียนว่า "ยอดเงินข้างบนเป็นของเฉพาะที่ดึงมา"
                  ⇒ ทั้งสองบรรทัดเคยถูก ตอนที่จอยังไม่มียอดกระจก · พอเพิ่มยอดกระจก บรรทัดนี้กลายเป็นเท็จ
                     **ในจอเดียวกัน** โดยไม่มีอะไรฟ้อง (คลาส new-capability-falsifies-old-copy)
                  ⚠️ อันตรายกว่าเขียนผิดเฉย ๆ เพราะคนอ่านเจอสองคำตอบแล้ว**ไม่รู้ว่าจะเชื่ออันไหน**
                  ⇒ ผูกประโยคกับเงื่อนไขเดียวกับที่หัวจอใช้ตัดสิน (`ใช้ยอดกระจก`) ⇒ สวนกันอีกไม่ได้ */}
          ...
          <AdvancedSearch
            ...
            notAvailable={[
              { what: 'สถานะ / การชำระเงิน', why: 'ท่อยังไม่รับเป็นตัวกรอง — ยิงยืนยัน 18 ก.ย. 2569: ส่ง status= และ paystatus= ไป total ค้าง 693 เท่าเดิมและไม่โผล่ใน ignored ⇒ ท่อไม่รู้จักชื่อนี้ · กรองในเบราว์เซอร์จะกรองได้แค่หน้าที่เห็น จึงไม่ทำ' },
            ]}
            extraNote={
          ...
          {d?.applied?.source === 'mirror' && (
            <p className="text-[12px] text-gray-600 bg-gray-50 border border-gray-200 rounded-md px-3 py-2 mb-3 leading-relaxed">
              ...
              {/* 🔴 **ตัวเทียบคำค้นเคยอยู่ตรงนี้ — ซึ่งซ้อนอยู่ใน `source === 'mirror'`** (ย้ายออก 19 ก.ย. 2569)
                  ⇒ รอบที่ท่อตอบจาก ZORT สด **ตาข่ายไม่ทำงานเลย** ทั้งที่ควรทำงานทุกรอบ
                  🔑 ตาข่ายที่ซ่อนอยู่ในเงื่อนไขอื่น = ตาข่ายที่หายไปในครึ่งหนึ่งของกรณี
                     และไม่มีอะไรบอกว่ามันหายไป ⇒ ย้ายไปเป็นก้อนของตัวเองข้างล่าง */}
            </p>
          )}

          {/* 🔴 **ตาข่ายตัวกรอง — ต้องอยู่นอกเงื่อนไขอื่นทั้งหมด** (เพิ่ม 19 ก.ย. 2569)
              เทียบ applied แล้ว: q from to
              ⚠️ วัดแล้ววันเดียวกัน: ท่อกรองทั้งสามจริง ⇒ ก้อนนี้เป็นตาข่ายเผื่ออนาคต ปกติไม่ขึ้น
              ⚠️ เทียบเฉพาะตอนผู้ใช้ใส่ค่านั้นจริง — ไม่ใส่แล้วท่อคืน null เป็นเรื่องปกติ */}
          {ตัวกรองที่ท่อไม่ได้ใช้.length > 0 && (
            <p className="text-[12px] text-amber-900 bg-amber-50 border border-amber-300 rounded-md px-3 py-2 mb-3 leading-relaxed">
