...
//
// 🔴 **ข้อความเดิม "ระบบเราเก็บผู้ใช้ใน env ไม่มีฐานข้อมูลผู้ใช้" กลายเป็นเท็จแล้ว** (เจอ 15 ก.ย. 2569 · ใบ t_mu1wvgcw)
//    ตอนนี้มีทะเบียนผู้ใช้จริงที่ `lib/staff-users.ts` + `/api/staff-users` และฟอร์มเพิ่มผู้ใช้
//    ที่ `/core/settings-users/add` (กดบันทึกแล้วเข้าใช้งานได้เลย · เก็บ `createdAt` ด้วย)
//    🔴 **และจอนี้ยังอ่านจาก env อย่างเดียว ⇒ ผู้ใช้ที่เพิ่มจากหน้าเว็บจะไม่โผล่ในตารางนี้เลย**
//       ⇒ คนเพิ่มผู้ใช้เสร็จแล้วกลับมาดู ไม่เห็นชื่อ ⇒ นึกว่าไม่สำเร็จ แล้วเพิ่มซ้ำ
//       ⇒ และ "จำนวน N รายการ" บนหัวจอก็นับไม่ครบ (กฎ CLAUDE.md: ตัวนับกับตัวแถวคนละกติกา)
//    ⇒ รอบนี้ **เขียนบอกบนจอตรง ๆ ก่อน** (ใบนี้เป็นใบไล่ข้อความค้าง ไม่ใช่ใบแก้พฤติกรรม)
//       การทำให้ตารางรวมผู้ใช้จากเว็บด้วยเป็นงานแยก — แจ้งฝั่ง CEO แล้ว
//
    ...
    <div className="p-4 md:p-6">
      <PageHead
        ...
        summary={<>จำนวน {rows.length} รายการ{' | '}
          <span className="text-gray-400">นับเฉพาะผู้ใช้ที่ตั้งใน Netlify env — <b>ยังไม่รวมผู้ใช้ที่เพิ่มจากหน้าเว็บ</b></span></>}
        actions={
            ...
            <tbody>
              {rows.map((r, i) => (
                <tr key={i} className="border-b border-[#e8ecf8] last:border-0">
                  ...
                  <td className={`${TD} text-gray-300`} title="ผู้ใช้แถวนี้ตั้งใน env จึงไม่มีวันสมัครบันทึกไว้">—</td>
