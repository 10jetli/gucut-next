...
export default function PrintLabelsPage() {
  ...
  /* มาจากปุ่ม "พิมพ์เอกสาร" ⇒ ?sku=<รหัส,รหัส,…> แล้วโหลดให้เลย
     🔴 **แยกด้วยคอมมาเท่านั้น ห้ามแยกด้วยช่องว่าง** (แก้ 15 ก.ย. 2569)
        ...
        🔴 **ทั้งสองทางเข้าใช้กติกาเดียวกัน** — ทั้งตรงนี้และ `skus` ที่อ่านจากช่องพิมพ์เอง
           แยกด้วย **บรรทัดใหม่หรือคอมมาเท่านั้น** ห้ามแยกด้วยช่องว่างทั้งคู่
           ⚠️ ร่างแรกของคอมเมนต์นี้เขียนว่า "ช่องพิมพ์เองยังแยกด้วยช่องว่างได้ตามเดิม"
              ซึ่ง **ผิดตั้งแต่วินาทีที่เขียน** เพราะ commit เดียวกันนั้นแก้ `skus` ไปแล้ว
              ⇒ คนอ่านรอบหน้าจะ "แก้โค้ดให้ตรงคอมเมนต์" แล้วบั๊ก 34 รหัสกลับมาทันที
              ⇒ เป็นบทเรียนเดียวกับที่ไล่แก้มาทั้งสัปดาห์ แต่รอบนี้เกิดในคอมเมนต์ของตัวเอง
                 (ฝั่งท่อจับได้ตอนอ่าน commit — code review จับได้ ตาข่ายจับไม่ได้
                  เพราะตาข่ายอ่านเฉพาะข้อความที่ขึ้นจอ ไม่ได้อ่านคอมเมนต์) */
  useEffect(() => {
    const s = qs.get('sku')
    if (s) {
