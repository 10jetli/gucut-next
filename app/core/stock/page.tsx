                ...
                {rows.map((r, i) => (
                  <tr key={r.sku} className="border-b border-gray-100 last:border-0 hover:bg-gray-50">
                    ...
                    <td className={`${TD} text-right`}>
                      {/* ⚠️ เมนู ⋮ ของ ZORT มี **15 รายการ** — ฝั่งเซิร์ฟเวอร์ไล่กดอ่านมาครบแล้ว
                          12 ใน 15 เป็น "สั่งให้ทำ" (ซื้อ · ขาย · โอน · ปรับจำนวน · ลบ · ปิดใช้งาน ·
                          ดันขึ้นช่องทางขาย) ซึ่งต้องมีท่อเขียนกลับ ZORT ก่อน
                          ⇒ **โชว์ครบตามผัง แต่ตัวที่ทำไม่ได้เป็นสีเทาพร้อมเหตุผล**
                             ตัดทิ้ง = คนที่ชิน ZORT หาไม่เจอแล้วนึกว่าระบบเราทำไม่ได้ */}
                      <RowMenu
                        items={[
                          {
                            label: 'ปักหมุดไว้บนสุด',
                            disabled: 'ZORT ไม่เปิด API หมวดหมู่/การปักหมุด — ปักได้ที่ ZORT เท่านั้น',
                          },
                          { label: 'ดูภาพรวม', onClick: () => { window.location.href = `/core/stock/${encodeURIComponent(r.sku)}` } },
                          { label: 'คัดลอกรหัสสินค้า', onClick: () => { navigator.clipboard?.writeText(r.sku).catch(() => {}) } },
                          {
                            label: 'ซื้อสินค้า',
                            disabled: 'ยังไม่มีท่อเปิดใบซื้อ — เปิดใบซื้อที่ ZORT ก่อน แล้วรอบซิงก์ถัดไปจะเข้ามาเอง',
                          },
                          { label: 'ขายสินค้า', onClick: () => { window.location.href = '/core/pos' } },
                          {
                            label: 'โอนสินค้า',
                            onClick: () => { window.location.href = '/catalog/index.html#trf' },
                          },
                          {
                            // 🔴 ห้ามชี้ไป /core/moves จากจอกระจก — ดูเหตุผลเต็มในหน้ารายละเอียดสินค้า
                            label: 'ปรับจำนวน',
                          ...
                          {
                            label: 'อัพเดทจำนวนไปช่องทางขายอื่น',
                            disabled: 'ยังดันสต็อกกลับไปมาร์เก็ตเพลสไม่ได้ — ต่อ Shopee แล้ว รอ Lazada/TikTok',
                          },
                          {
                            label: 'อัพเดทข้อมูลสินค้าไปช่องทางขายอื่น',
                            disabled: 'ยังดันข้อมูลสินค้ากลับไปมาร์เก็ตเพลสไม่ได้',
                          },
