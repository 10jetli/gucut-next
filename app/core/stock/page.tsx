...
interface Resp {
   ...
   *  ✅ **แก้แล้วฝั่งท่อ 19 ก.ย. 2569** (gucut-web b393950) — เส้นนี้เติม `applied` + `appliedNote`
   *     เหตุ: ฝั่งท่อวางเส้นทั้ง 24 เส้นเรียงกันแล้วเห็นว่า 11 เส้นส่ง `applied` อีก 13 ไม่ส่ง
   *     และสองเส้นที่ใหญ่ที่สุด (`list=orders` · `list=stock`) **กรองด้วย `q` จริง (2,674 → 0)
   *     แต่ไม่บอกว่ากรอง** ⇒ จอเขียนได้แค่ "ไม่พบ" โดยไม่บอกว่าไม่พบอะไร
   *  ⚠️ ยังไม่ deploy ตอนที่เขียนจอนี้ ⇒ **ไม่มีคีย์ = ไม่เขียนคำกำกับ** (ไม่ใช่เดาว่ากรองแล้ว) */
  applied?: { q?: string | null; category?: string | null; sort?: string | null; only?: string | null; kind?: string | null } | null
  appliedNote?: string
  only?: string | null
  kind?: string | null
...
/* ตัวเลือกเรียง — **ลอกความสามารถจาก ZORT** (อ่านของจริง 18 ก.ย. 2569)
   จอ ZORT เรียงด้วยลิงก์ `mysort=` บนหัวคอลัมน์ 6 ช่อง:
     code · name · purchaseprice · sellprice · stock · availablestock
   ของเรามีแค่ 3 ตัวมาตลอด ทั้งที่ท่อรองรับ 10 แบบ (sortsSupported ตอบกลับมาเอง)
   ⇒ เติมให้ครบเท่าที่ ZORT ทำได้ · ไม่เพิ่มของที่ ZORT ไม่มี (เจ้าของร้านสั่งให้เหมือน ZORT)
   ⚠️ `sold` (ขายดีก่อน) เป็นของเราเอง ZORT ไม่มี — **เก็บไว้** เพราะมีมาก่อนและร้านใช้จริง
   ⚠️ ป้ายต้องเป็นคำที่คนอ่านเข้าใจ ไม่ใช่ชื่อค่าดิบของ API */
const SORTS = [
  { id: 'qty', label: 'ของใกล้หมดก่อน' },
  { id: 'qtydesc', label: 'ของเยอะก่อน' },
  { id: 'availablelow', label: 'พร้อมขายน้อยก่อน' },
  { id: 'sold', label: 'ขายดีก่อน' },
  { id: 'sku', label: 'เรียงตามรหัส' },
  { id: 'name', label: 'เรียงตามชื่อ' },
  { id: 'price', label: 'ราคาขายมากก่อน' },
  { id: 'pricelow', label: 'ราคาขายน้อยก่อน' },
  { id: 'buy', label: 'ราคาซื้อมากก่อน' },
]
...
function CoreStockInner() {
  ...
  /* 🔎 **แถวที่คนดูมองไม่เห็นว่าทำไมถึงติดคำค้น** (วัดของจริง 16 ก.ย. 2569 · ใบ t_mu2u9mym)
     ...
     ✅ **รู้แล้วว่าช่องไหน (17 ก.ย. 2569) — คือ "ชื่อสินค้าที่พิมพ์ไว้ในใบขายเก่า"**
        อ่านจากซอร์สของท่อ (`gucut-web/netlify/lib/core-stock.mjs`): `q` ค้น 3 ที่ —
        `cur.sku LIKE` · `products.name LIKE` · **`order_items.name LIKE`** (ชื่อบนบรรทัดของใบขาย)
        🔬 ยืนยันด้วยของจริง: รหัส `03409-3` ชื่อปัจจุบันคือ "ตะไบ NEWWAVE"
           แต่บรรทัดในใบขายเก่าเขียนว่า "ตะไบ NEWWAVE ตะไบหางหนู **ลับฟันโซ่เลื่อย**" ⇒ จึงติดคำค้น "โซ่"
        ⇒ **ไม่ใช่ความผิดพลาด** — เป็นการค้นที่ครอบชื่อเดิมของสินค้าด้วย (มีประโยชน์ตอนลูกค้าเรียกชื่อเก่า)
           แค่ต้องบอกคนดูว่าทำไมแถวนี้ถึงโผล่
        📌 ตัดออกไปแล้วระหว่างทาง (อย่าลองซ้ำ): ชื่อหมวดหมู่ · ช่องในรายละเอียดสินค้าของท่อ · การอยู่ในชุดสินค้า
    ...
    <div className="p-4 md:p-6">
        ...
        summary={
          ...
          : data ? (
            <>
              ...
              {/* 🔴 **เลขนี้เคยเป็นของทั้งคลัง ทั้งที่ตารางข้างล่างถูกกรองแล้ว** (แก้ 19 ก.ย. 2569)
                  วัดจริง: `list=stock&only=out` ⇒ `total` = 2,674 (ทั้งคลัง) แต่ `rowsMatched` = 570
                  ⇒ หัวตารางบอก 2,674 · แท็บบอก 570 · ตารางมี 570 ⇒ **ตัวนับกับตัวแถวคนละกติกา**
                  ตรงกับกับดักข้อ 4 ใน CLAUDE.md ที่เราจดไว้เอง (และเคสตัวอย่างในนั้นคือจอนี้พอดี)
                  ⇒ ใช้เลขของแท็บที่กำลังดู **และเขียนของทั้งคลังกำกับเมื่อสองเลขต่างกัน**
                  🔑 ไม่ใช่แค่สลับตัวเลข — ถ้าสลับเงียบ ๆ คนที่เคยเห็น 2,674 จะคิดว่าของหายไป 2,104 ชิ้น
                  📌 รูปประโยค "จำนวน N รายการ" ยังคงเดิมตามที่เจ้าของร้านสั่ง (ลอก ZORT เป๊ะ) */}
              {typeof data.total !== 'number'
                ? <>จำนวน — รายการ (เซิร์ฟเวอร์ไม่ได้ส่งจำนวนมา)</>
                : inTab === data.total
                  ? <>จำนวน {data.total.toLocaleString('th-TH')} รายการ</>
                  : <>จำนวน {inTab.toLocaleString('th-TH')} รายการ{' '}
                      <span className="text-gray-400">(จากทั้งคลัง {data.total.toLocaleString('th-TH')})</span></>}
              ...
              {Number(data.noSkuInZort) > 0 && !category && !q.trim() && tab === 'all' && (
                /* 🔴 **ต้องหักให้ลงตัวกับเลขที่โชว์อยู่ ไม่ใช่หักกับเลขอื่น** (แก้ 18 ก.ย. 2569)
                   ยิงพิสูจน์เอง: `list=stock` ได้ 2,674 · `list=stock&kind=goods` ได้ 2,668 (บริการ 6)
                   แต่แท็บเริ่มต้นของจอนี้คือ **เฉพาะสินค้า** ⇒ หัวจอโชว์ 2,668
                   ⇒ ป้ายเดิมเขียนว่า "ZORT มี 2,900 · ไม่มีรหัส 226" ซึ่งหักแล้วได้ 2,674
                     **ไม่ใช่ 2,668 ที่อยู่หน้าประโยคเดียวกัน** ⇒ เหลือ 6 ตัวที่ไม่มีคำอธิบาย
                   ⇒ คนที่เอาไปเทียบจะหาของหาย 6 ตัวที่ไม่เคยหาย (กฎข้อ 4 ใน CLAUDE.md)
                   ⇒ เขียนการหักให้ครบทุกก้อนจนเหลือเศษศูนย์ */
                <span className="text-gray-400">
                  {' '}(ZORT มี {(data.zortTotal ?? 0).toLocaleString('th-TH')} — หัก
                  {' '}<b>ไม่มีรหัส {Number(data.noSkuInZort).toLocaleString('th-TH')} ตัว</b>
                  {typeof data.noSkuWithStock === 'number' && data.noSkuWithStock === 0 && <> (สต็อกเป็นศูนย์ทั้งหมด)</>}
                  {/* แท็บเริ่มต้นไม่รวม "บริการ" ⇒ ต้องหักออกด้วย ไม่งั้นเลขไม่ลงตัวกับที่โชว์ */}
                  {kind === 'goods' && typeof data.services === 'number' && data.services > 0 && (
                    <> · หัก<b>บริการ {data.services.toLocaleString('th-TH')} รายการ</b> ที่แท็บนี้ไม่รวม</>
                  )}
                  )
                </span>
              )}
              ...
              {/* 🔴 **คำสั่งนี้เคยพาไปที่ที่ทำไม่จบ** (แก้ 18 ก.ย. 2569 · ใบ S4)
                  เดิมเขียนว่า "กดรหัสสินค้า แล้วกดแก้ไข" เพื่อใส่รูป
                  แต่จอแก้ไขสินค้า **ปิดปุ่มส่งจริงอยู่** (รอท่านประธานอนุมัติจอนั้นโดยเฉพาะ)
                  ⇒ คนทำตามแล้วซ้อมได้ แต่รูปไม่เข้า ZORT ⇒ คู่มือที่ทำตามแล้วไม่จบ แย่กว่าไม่มีคู่มือ
                  ⚠️ วันที่ท่านประธานเปิดจอนั้น **ต้องกลับมาแก้ประโยคนี้ด้วย** */}
              <span className="text-gray-600">จัดการรูปภาพสินค้า: ตอนนี้ใส่รูปเข้า ZORT จากจอเรา<b>ยังไม่ได้</b>
                {' '}(จอแก้ไขสินค้าซ้อมได้ แต่ปุ่มส่งจริงยังปิด รอท่านประธานอนุมัติ)
                {' '}· ใส่รูปได้ที่จอ <b>&ldquo;จัดการรูปภาพสินค้า&rdquo; ของ ZORT</b> (เมนูสินค้า) ซึ่งใส่หลายรหัสทีเดียวได้</span>
              {' | '}
      ...
      {แถวไม่เห็นเหตุ > 0 && (
        <p className="text-[12.5px] text-gray-600 bg-gray-50 border border-gray-200 rounded px-3 py-2 mb-3">
          🔎 ในหน้านี้มี <b>{แถวไม่เห็นเหตุ.toLocaleString('th-TH')} รายการ</b> ที่ <b>รหัสและชื่อปัจจุบันไม่มีคำค้น</b> —
          แต่ตรงกับ <b>ชื่อสินค้าที่พิมพ์ไว้ในใบขายเก่า</b> (ติดป้าย “ตรงที่ช่องอื่น” ไว้ให้ทีละแถว)
          <span className="block mt-0.5 text-gray-500">
            ⇒ จำนวนที่เจอจะ<b>มากกว่าจอ ZORT</b> ซึ่งค้นเฉพาะรหัส/ชื่อ (วัดคำว่า “โซ่” 16 ก.ย. 2569: ZORT 90 · ของเรา 156)
            {' '}· ตัวอย่างจริง: รหัสหนึ่งชื่อปัจจุบันว่า “ตะไบ NEWWAVE” แต่ในใบขายเก่าเขียนว่า “…ลับฟันโซ่เลื่อย”
          </span>
        </p>
      )}
        ...
        <>
            ...
            <table className="w-full min-w-[920px]">
              <thead className="bg-white border-b border-gray-200">
                ...
                <tr>
                  ...
                  <th className={THR}>
                    {/* 🔴 **ผมเขียนผิดเองเมื่อชั่วโมงก่อน แล้วแก้แล้ว — 18 ก.ย. 2569**
                        รอบแรกผมเขียนว่า "ไม่ยืนยันว่าจอ ZORT กดเรียงช่องนี้ได้ — หัวตารางไม่มีตัวสั่งเรียง"
                        ⇒ **ผิด** · ของจริง ZORT เรียงด้วย **ลิงก์ `mysort=` บนหัวคอลัมน์** ไม่ใช่ onclick
                        ตัววัดของผมรอบนั้นมองหาแต่ `onclick` กับคลาสที่มีคำว่า sort ⇒ **มองไม่เห็นลิงก์**
                        (คลาสเดียวกับบทเรียน "เครื่องมือทดสอบมองไม่เห็นมิติที่ตัวเองไม่มี" ที่เพิ่งจดวันนี้
                         — เจอซ้ำภายในชั่วโมงเดียวกัน)
                        📏 ของจริง: ZORT เรียงได้ 6 คอลัมน์ — code · name · purchaseprice · sellprice ·
                           stock · availablestock ⇒ ช่องนี้ (พร้อมขาย) **ZORT เรียงได้จริง**
                        ⇒ ใส่เป็นตัวเลือกเรียงแล้วใน SORTS (ดูหมายเหตุที่นั่น) */}
                    <span title="เรียงตามช่องนี้ได้ที่ตัวเลือก &quot;เรียง&quot; ด้านบน (ZORT เรียงช่องนี้ได้เหมือนกัน — mysort=availablestock)">พร้อมขาย</span>
                  </th>
              ...
              <tbody>
                ...
                {rows.length === 0 && (
                  tab === 'inactive'
                    ...
                    : <EmptyState cols={10} icon="🔍"
                        /* 🔴 **เลขศูนย์ต้องมีขอบเขตกำกับ** — "ไม่พบสินค้า" เฉย ๆ อ่านได้ว่า "คลังว่าง"
                           ฝั่งท่อเจอ 19 ก.ย. 2569 ว่า `list=stock` **กรองด้วย `q` จริง (2,674 → 0)
                           แต่ไม่ได้บอกเลยว่ากรอง** ⇒ จอจึงพูดได้แค่ "ไม่พบ" โดยไม่บอกว่าไม่พบ**อะไร**
                           ⇒ ท่อเติม `applied.q` ให้แล้ว ⇒ จอเขียนกำกับได้
                           🔑 **อ่านจากที่ท่อสะท้อนกลับ ไม่ใช่จากคำที่จอส่งไป** (ส่งได้ ≠ มีผล)
                              ท่อรุ่นเก่าไม่มี `applied` ⇒ ไม่เขียนคำกำกับ (ดีกว่าอ้างว่ากรองแล้วทั้งที่ยังไม่รู้) */
                        title={data?.applied?.q
                          ? `ไม่พบสินค้าที่ตรงกับคำค้น “${data.applied.q}”`
                    ...
                    <td className={TD}>
                      ...
                      {!ตรงที่เห็นบนจอ(r) && (
                        <span
                          ...
                          title="แถวนี้ไม่มีคำค้นอยู่ในรหัสหรือชื่อปัจจุบัน — แต่ตรงกับ **ชื่อสินค้าที่พิมพ์ไว้ในใบขายเก่า** (ท่อค้นชื่อในบรรทัดใบขายด้วย) · จอ ZORT ค้นคำเดียวกันจะไม่เจอแถวนี้ · ไม่ใช่ความผิดพลาด — มีประโยชน์ตอนลูกค้าเรียกชื่อเก่าของสินค้า"
                        >
                          ตรงที่ช่องอื่น
                        </span>
                      )}
          ...
          {picked.length > 0 && (
            <BulkBar>
              ...
              {/* 🔴 เดิมเขียนเหตุเดียวเหมารวมทั้ง 5 อย่างว่า "ท่อยังไม่เปิดให้เขียนเป็นชุด"
                  ยิงตรวจจริง 17 ก.ย. 2569 แล้วพบว่า **ติดคนละเหตุ** และสองอันติดที่ฝั่ง *อ่าน* ไม่ใช่ฝั่งเขียน
                  ⇒ เหตุผลรวม ๆ ทำให้คนรอฝั่งที่ไม่ใช่ต้นตอ (โรคประจำโปรเจกต์: ระบบถูก แต่สื่อสารผิด) */}
              <span className="text-gray-500 basis-full">
                ⚠️ เมนู “คำสั่ง” ของ ZORT ยังมีอีก 5 อย่างที่<b>เรายังทำไม่ได้</b> — แต่<b>ติดคนละเหตุ</b>:
                <br />• <b>ซื้อสินค้า · ขายสินค้า · โอนสินค้า</b> — ต้องเขียนกลับไปที่ ZORT เป็นชุด ท่อยังไม่เปิด
                <br />• <b>จัดหมวดหมู่</b> — ท่อเก็บหมวดของสินค้าไว้ครบแล้ว (จอนี้<b>กรองตามหมวดได้จริง</b>)
                แต่ยัง<b>ไม่ส่งหมวดมาพร้อมแต่ละแถว</b> ⇒ จัดหมวดแล้วดูผลบนจอนี้ไม่ได้ · ขอฝั่งท่อไว้แล้ว
                <br />• <b>เพิ่ม Tag</b> — หนักกว่าเพื่อน: <b>ท่อยังไม่มีที่เก็บแท็กเลย</b>
                (ยิงตรวจ 17 ก.ย. 2569 — แถวสินค้าที่ท่อส่งมาไม่มีช่องแท็ก)
                ⇒ ต่อให้ติดแท็กได้ ก็จะเห็นได้เฉพาะในจอ ZORT ไม่ใช่จอนี้
              </span>
            </BulkBar>
          )}
