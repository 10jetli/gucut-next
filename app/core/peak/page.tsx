...
//
// 🔴 **ถ้าวันหนึ่งมีคนทำปุ่มส่งจริงที่จอนี้ — อ่านตรงนี้ก่อน** (ฝั่งท่อแจ้งล่วงหน้า 19 ก.ย. 2569)
//    เส้น **ส่งจริง** ของ PEAK **ห้ามตัดสินผลจากรหัส HTTP** ⇒ ให้อ่านช่อง `สรุป` แทน
//    เหตุ: ฝั่งท่อเจอว่าของเดิมเขียน `r.ok ? 200 : …` ⇒ ส่ง 5 ใบ สำเร็จ 3 ล้ม 2
//    **ตอบ 200 แล้วกลืนสองใบที่ล้ม** (ไม่เข้าบัญชี และไม่มีใครรู้)
//    ⇒ เขาเปลี่ยนให้ "สำเร็จบางส่วน" ตอบ **502** และเพิ่มสถานะ
//       "PEAK ตอบมาไม่ครบจำนวนใบที่ส่ง" ซึ่งเดิมไม่มี
//       (ส่ง 3 ตอบ 1 ที่ผ่านหมด เคยถูกอ่านว่า "สำเร็จครบ" ⇒ สองใบหายจากบัญชี)
//    🔑 ⇒ **ความสำเร็จบางส่วนไม่มีรหัส HTTP ที่ถูกต้อง** — 200 โกหก 502 ก็ยังไม่บอกว่าใบไหนล้ม
//       ของที่ตอบได้คือเนื้อในคำตอบเท่านั้น
//
// ✅ **วันนี้จอนี้ไม่ได้รับผลกระทบ** — เรียกแค่ `peak=status` กับ `peak=dry`
//    (ตรวจแล้ว 19 ก.ย. 2569 · ทั้งรีโปไม่มีที่ไหนเรียกเส้นส่งจริงเลย)
//    ⚠️ แต่สองเส้นนั้นจอยังตัดสินด้วย `res.ok` อยู่ ⇒ **วันที่ท่อเอาแนวคิดเดียวกันไปใช้กับ
//       สองเส้นนี้ จอจะโยน error ใส่ผลซ้อมที่สำเร็จบางส่วน** ⇒ ต้องเปลี่ยนมาอ่านเนื้อในด้วย
