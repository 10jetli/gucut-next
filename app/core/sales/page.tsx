...
interface ListResp {
  ...
  /** ✅ ท่อเติมให้ 19 ก.ย. 2569 (gucut-web b393950) — ค่าที่ท่อ **ใช้จริง** ในคำขอนี้
   *  🔴 เหตุ: เส้นนี้กรองด้วย `q` จริง (868 → 0) **แต่ไม่เคยบอกว่ากรอง** ⇒ จอเขียนได้แค่ "ไม่พบ"
   *  ⚠️ **ไม่มีคีย์ = ยังไม่ deploy** ⇒ จอไม่เขียนคำกำกับ (ไม่ใช่เดาว่ากรองแล้ว) */
  applied?: { q?: string | null; status?: string | null; channel?: string | null; store?: string | null } | null
  appliedNote?: string
...
export default function CoreSalesPage() {
  ...
  /* 🔴 **สถานะที่ท่อมี แต่เรายังไม่รู้ว่า ZORT เรียกเป็นไทยว่าอะไร** (เจอของจริง 18 ก.ย. 2569)
     พอท่อเริ่มส่ง `statusesAll` (สารบัญทั้งระบบ ไม่กรองวัน) แท็บ **`Returned`** ก็โผล่ขึ้นจอ
     เป็นภาษาอังกฤษดิบ — ซึ่งเป็นอาการเดียวกับที่ท่านประธานทักไว้ ("Pending" ในคอลัมน์สถานะ)
     🔑 ของเก่าไม่ได้พัง: `zort-words` **ตั้งใจไม่เดาคำ** และคำนี้อยู่ใน `UNKNOWN_FROM_PIPE` มาตลอด
        มันแค่ไม่เคยถูกมองเห็น เพราะ `byStatus` กรองด้วยช่วงวัน และใบเดียวที่มีอยู่คือ ก.พ. 2568
     ⇒ **ความสามารถใหม่ทำให้ช่องโหว่เก่าที่ถูกต้องอยู่แล้ว กลายเป็นของที่คนเห็น**
     ⚠️ ห้ามเติมคำไทยเองเพื่อให้จอสวย (กฎ `/zort-words`: คำที่ระบบไม่รองรับ = จอโกหก)
        ⇒ โชว์คำดิบพร้อมเครื่องหมาย แล้วเขียนบอกข้างล่างว่ายังไม่ยืนยัน · รอ CEO อ่านจาก ZORT
     ✅ บรรทัดคำอธิบาย **ผูกกับเงื่อนไขนี้** ⇒ วันที่เติมคำลง SALE_STATUS มันหายเอง
        (ข้อความที่ต้องรอคนมาถอด คือข้อความที่จะค้าง) */
  const สถานะที่ยังไม่มีคำไทย = รายชื่อสถานะ.filter((st) => !zortWord(SALE_STATUS, st).known)
  const tabs = [
    { id: '', label: 'ทั้งหมด', count: allCount || data?.total },
    ...รายชื่อสถานะ.map((st) => ({
      id: st,
      label: zortWord(SALE_STATUS, st).known ? statusTh(st) : `${st} ⚠️`,
      count: นับสถานะ(st),
    })),
  ]
  ...
  return (
    <div className="p-4 md:p-6">
      ...
      {/* 🔴 **ตารางนี้ผูกกับ cron ของท่อโดยตรง** — ท่อลด core-sync จาก `13,43` เหลือ `13` (ทุกชั่วโมง)
          เมื่อ 18 ก.ย. 2569 ⇒ ถ้าไม่ส่งค่าใหม่ จอจะใช้ค่าตั้งต้น 30 นาที แล้ว
          ① เขียนตารางที่ไม่มีอยู่จริง ("นาทีที่ 13 และ 43")
          ② เตือนเร็วเกินจริง (เกณฑ์ = รอบ × 3 + 10 ⇒ 100 นาที ทั้งที่รอบจริงห่างกัน 60 นาที)
          ⇒ ส่งค่าจริงเข้าไป · ด่าน check-cron-table จับเฉพาะตารางบนจองานตามเวลา **ไม่ครอบจุดนี้** */}
      <DataFreshness freshness={data?.freshness} everyMinutes={60} scheduleNote="นาทีที่ 13" />
      ...
      {data && !data.skip && (
        <>
          ...
          {/* คำดิบที่ยังไม่มีคำไทยยืนยัน — บรรทัดนี้หายเองวันที่เติมคำลง `lib/zort-words.ts` */}
          {สถานะที่ยังไม่มีคำไทย.length > 0 && (
            <p className="mt-1 text-[11.5px] text-amber-800">
              ⚠️ แท็บ <b>{สถานะที่ยังไม่มีคำไทย.join(' · ')}</b> ยังเป็น<b>คำดิบจาก ZORT</b> —
              เรายังไม่ได้ยืนยันว่าจอ ZORT เรียกสถานะนี้เป็นไทยว่าอะไร จึง<b>ไม่เดาคำแปล</b>
              <span className="text-amber-700/80"> (คำแปลผิดบนคอลัมน์สถานะ ทำให้คนแพ็กของอ่านใบผิด)</span>
            </p>
          )}
          ...
          <TableWrap>
            <table className="w-full min-w-[1080px]">
              ...
              <tbody>
                {rows.length === 0 && (
                  <EmptyState cols={12} icon="🧾"
                    /* 🔴 **เลขศูนย์ต้องมีขอบเขตกำกับ** — "ไม่พบใบขาย" เฉย ๆ อ่านได้ว่า "ไม่มีออเดอร์เลย"
                       ฝั่งท่อวัดเอง 19 ก.ย. 2569: `q=zzzz` ⇒ total **868 → 0** และคำตอบ**ไม่มีคีย์ `q` เลย**
                       ⇒ คนอ่านสรุปว่าไม่มีออเดอร์ ทั้งที่มี 868 ใบ · ท่อเติม `applied` ให้แล้ว (b393950)
                       🔑 **อ่านจากที่ท่อสะท้อนกลับ ไม่ใช่จากคำที่จอส่งไป** — จอรู้ว่าตัวเองพิมพ์อะไร
                          แต่ไม่รู้ว่าท่า**ใช้**คำนั้นจริงไหม (ส่งได้ ≠ มีผล) ⇒ ไม่มี `applied.q` ⇒ ไม่เขียนกำกับ */
                    title={data?.applied?.q
                      ? `ไม่พบใบขายที่ตรงกับคำค้น “${data.applied.q}”`
                      : 'ไม่พบใบขายในเงื่อนไขนี้'}
