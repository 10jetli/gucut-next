...
//    แก้ไข/อัปรูป/เปลี่ยนสถานะ ยังทำที่ ติดตามสินค้าจากจีน กับ ดรอปชิปปิ้ง เหมือนเดิม
...
/* 🔴 **รายการตายตัวที่ของใหม่จะหายเงียบ ๆ** (แก้ 5 ก.ย. 2569 ตอนไล่ตรวจทั้งระบบ)
   เดิมเขียนรายชื่อสถานะ "ยังไม่จบ" ไว้ตรง ๆ ⇒ วันที่มีสถานะใหม่เพิ่มในระบบ
   ใบที่อยู่สถานะนั้นจะ **หายจากยอดรวมและเงินมัดจำทันที** โดยไม่มี error และไม่มีใครรู้
   (TypeScript ไม่ช่วยเลย เพราะอาร์เรย์ที่ขาดสมาชิกยังถูกต้องตามชนิดข้อมูล)
   ⇒ กลับด้าน: ประกาศเฉพาะสถานะที่ถือว่า **จบแล้ว** ที่เหลือถือว่ายังไม่จบโดยอัตโนมัติ
   ⇒ สถานะใหม่จะถูกนับรวมไว้ก่อน (ปลอดภัยกว่าหายไปเงียบ ๆ) แล้วค่อยมาตัดสินทีหลัง
   ⚠️ `warehouse` (รอเข้าโกดัง) ยังถือว่า "จบแล้ว" ตามนิยามเดิม — ยังไม่เปลี่ยน
      เพราะมันขยับตัวเลขเงิน ต้องให้เจ้าของร้านตัดสินว่าของที่ยังไม่เข้าโกดังนับเป็นงานค้างไหม */
const CLOSED: TrackerStatus[] = ['warehouse', 'done']
const OPEN: TrackerStatus[] = (Object.keys(STATUS_LABEL) as TrackerStatus[])
          ...
          <TableWrap>
            ...
            <div className="flex flex-wrap items-center justify-between gap-3 px-3 py-2.5 border-t border-gray-200 bg-white">
              ...
              <Link href="/tracker" className="text-[12px] text-blue-600 hover:underline">
                แก้ไข / อัปรูป / เปลี่ยนสถานะ ที่หน้าติดตามสินค้าจากจีน →
              </Link>
            </div>
          </TableWrap>
        </>
