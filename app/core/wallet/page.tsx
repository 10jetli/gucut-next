...
//    ZORT มี 46 ใบ ยอดรวมหลักสิบล้าน · แต่ **ไม่มีเส้นดึง "รายการชำระเงินทั้งหมด"**
//    🔴 หัวไฟล์นี้เคยเขียนว่า "ยิง API แล้วตอบ 404 ทุกเส้น (Wallet · Payment · Receipt)"
//       ซึ่ง **เนื้อจอด้านล่างแก้ไปแล้วตั้งแต่ 6 ก.ย.** แต่หัวไฟล์ไม่เคยตามไปแก้ (เจอ 15 ก.ย. 2569)
//       ⇒ ไฟล์เดียวกันพูดสองอย่าง · ฝั่งท่อกวาดซ้ำ 15 ก.ย. ยืนยันอีกชั้น:
//         `Payment/GetPaymentDetail` 200 (ทีละใบ) · `Payment/AddPayment` 405 ·
//         `Merchant/GetPaymentMethods` 200 (แค่ id/name ไม่มียอด) · `Wallet/*` `Receipt/*` 404 จริง
...
/* 📏 อัปเดต 18 ก.ย. 2569 (งานยืนไล่คำเตือนหมดอายุ): เปิดจอ ZORT อ่านใหม่
   จำนวนใบยัง 46 เท่าเดิม แต่ **ยอดคงเหลือขยับ** 11,532,894.2992 → 11,772,256.9592
   ⇒ ตัวเลขคัดมือชุดนี้เก่าไป 15 วันแล้วจริง ๆ (กลไกบอกอายุที่ตั้งไว้ทำงานถูกต้อง) */
const SEEN = { rows: 46, total: '11,772,256.9592' }
  ...
  return (
    <div className="p-4 md:p-6">
      ...
      <div className="bg-red-50 border border-red-200 rounded-md px-4 py-3 mb-3">
        ...
        {/* 🔴 **แก้ 6 ก.ย. 2569 — ข้อความเดิมกลายเป็นเท็จบางส่วน**
            เดิมเขียนว่า "404 ทุกเส้น (Wallet · Payment · Receipt)"
            แต่การกวาดคู่ (โมดูล × คำกริยา) 140+ ชื่อ พบว่า **โมดูล Payment มีอยู่จริง**
            (`Payment/GetPaymentDetail` รายใบ · `Payment/AddPayment`)
            ที่ไม่มีคือ **เส้นดึง "รายการชำระทั้งหมด"** ⇒ ยังทำจอนี้ไม่ได้เหมือนเดิม
            แต่เหตุผลคนละอย่าง และคนอ่านรอบหน้าต้องได้เหตุผลที่ถูก ไม่ใช่เหตุผลที่สะดวก
            ⚠️ นี่คือคลาส stale-claim: ข้อความที่เคยจริง แล้วกลายเป็นเท็จเมื่อความรู้เปลี่ยน
               และ **ไม่มีอะไรฟ้องเลย** ถ้าไม่มีใครกลับมาอ่าน */}
        <p className="text-[13px] text-red-900 mt-2 leading-relaxed">
          <b>ZORT ไม่เปิดเส้นดึง &ldquo;รายการชำระเงินทั้งหมด&rdquo;</b> — กวาดชื่อทั้งแผงแล้ว
          {' '}(6 ก.ย. 2569) เจอแต่ <code>Payment/GetPaymentDetail</code> ซึ่งดูได้<b>ทีละใบ</b>
          {' '}จากเลขที่ใบที่รู้อยู่แล้ว ⇒ <b>ไล่ออกมาทำเป็นตารางไม่ได้</b>
        </p>
        <p className="text-[12px] text-red-800 mt-1.5 leading-relaxed">
          ⚠️ <b>ขอบเขต:</b> &ldquo;ไม่พบภายใต้ชื่อที่กวาด&rdquo; ไม่ใช่ &ldquo;ไม่มีแน่นอน&rdquo; ·
          ดูรายการเส้นที่ยิงตรวจแล้วทั้งหมดได้ที่{' '}
