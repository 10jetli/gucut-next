...
interface VerifyResp {
  ...
  /** 🔴 **ช่องนี้หายเมื่อไหร่ ติ๊กเขียวกลับมาทันที และไม่มีอะไรบอก** (19 ก.ย. 2569)
   *
   *  จอตัดสินว่า "ตรวจไม่ได้" จากช่องนี้ช่องเดียว ⇒ ถ้าท่อเลิกส่ง (หรือรุ่นเก่าไม่เคยส่ง)
   *  `undefined` จะถูกอ่านว่า "ตรวจได้และไม่มีปัญหา" ⇒ **จอกลับไปหน้าตาเดิมก่อนมีฟีเจอร์นี้**
   *  ⇒ ซึ่งคือบั๊กที่เพิ่งแก้ไปเมื่อเช้าวันเดียวกัน — มันจะกลับมาเงียบ ๆ ไม่ใช่ขึ้นแดง
   *
   *  🔑 กติกาทั่วไปที่ฝั่งท่อสรุปไว้: **ฟีเจอร์ที่หายแล้วจอกลับไปหน้าตาเดิม
   *     ต้องมีตัวจับเสมอ** ไม่งั้นมันคือของที่หายได้โดยไม่มีใครรู้
   *  ⇒ ขอให้ท่อ **ส่งช่องนี้เสมอแม้เป็น false** แล้ว "ไม่มีคีย์" จะแปลว่าท่อรุ่นเก่าได้จริง
   *     (ท่าเดียวกับ `qtyUnreadableLines` ที่ฝั่งท่อทำให้ฟีดใบคืนวันนี้)
   *  ⚠️ จนกว่าจะส่งเสมอ **ห้ามเพิ่มเงื่อนไขที่ทำให้แถบนี้ขึ้นยากกว่าเดิม** — ตอนนี้มันเปราะอยู่แล้ว */
                  ...
                  {v && v !== 'busy' && (
                    v.error ? (
                      ...
                      <div className="text-[12px] mt-2 border-t border-gray-100 pt-2">
                        {/* 🔴 **ติ๊กเขียวห้ามขึ้นตอนที่ยังไม่ได้ตรวจ — และคำเตือนต้องอยู่บนสุด**
                            (เจอของจริง 19 ก.ย. 2569) ยิง `stockpushverify` แล้วท่อตอบ
                            `inconclusive:true` + note **"ตรวจไม่ได้ — ห้ามอ่านว่าผ่าน"**
                            แต่จอขึ้น `✅ ถึงหน้าร้านแล้ว 0 ตัว` สีเขียว ส่วนประโยคของท่อ
                            ไปอยู่ล่างสุดด้วยสีเทาจาง ⇒ อ่านได้ความว่า "ตรวจแล้ว ไม่มีอะไรค้าง"
                            ซึ่งตรงข้ามกับความจริง
                            🔑 **ศูนย์ที่แปลว่า "ยังไม่ได้ตรวจ" กับศูนย์ที่แปลว่า "ตรวจแล้วไม่มี"
                               ห้ามหน้าตาเหมือนกัน** · และของที่แรงที่สุดต้องอยู่บนสุด ไม่ใช่ต่อท้าย
                            ⚠️ รอบแรกผมวางแถบนี้ไว้ *ใต้* รายละเอียด — เห็นบนจอจริงถึงรู้ว่าผิดกฎตัวเอง
                               (build ผ่าน ≠ จอถูก · ต้องเปิดดูด้วยตา) */}
                        {v.inconclusive && (
                          <span className="block text-amber-800 font-semibold mb-1">
                            {/* note ของท่อขึ้นต้นด้วย "ตรวจไม่ได้" อยู่แล้ว ⇒ ไม่เติมคำของเราซ้ำ */}
                            ⛔ {v.note || 'รอบนี้ตรวจไม่ได้'}
                            <span className="block font-normal">
                              ⇒ ตัวเลขด้านล่างไม่ใช่ผลตรวจ · <b>ห้ามอ่านว่าผ่าน</b> และห้ามอ่านว่าไม่ผ่าน
                            </span>
                          </span>
                        )}
                        ...
                        {/* 🔴 **กองที่หายไประหว่างทาง ต้องถูกนับ ไม่ใช่ปล่อยให้เงียบ** (19 ก.ย. 2569)
                            ฝั่งท่อเพิ่งพบว่ารหัสที่ **ไม่มีในทะเบียนสินค้า** เคยถูกตอบว่า `landed`
                            (เงื่อนไขเดิม "ไม่อยู่ในกองใดเลย ⇒ ถึงแล้ว") ⇒ เขาเพิ่มด่านให้ตอบ
                            `unknown/not_in_warehouse` แทน — ซึ่ง **ไม่ใช่ทั้ง landed และ notLanded**
                            ⇒ จอนี้ที่อ่านแค่สองกอง จะทำรหัสพวกนั้น **หายไปเฉย ๆ** โดยไม่มีอะไรบอก
                            🔑 จอจึงต้องเช็คของตัวเองว่า "สองกองรวมกัน = ที่ส่งไปหรือเปล่า"
                        ...
                        {(() => {
                          ...
                          /* 🔴 **สองจุดที่จอนี้เคยพูดผิด** (เจอจากการยิงของจริง 19 ก.ย. 2569 ไม่ใช่จากอ่านซอร์ส)
                             ① ท่อส่ง `unknown[].reason` มาเสมอ แต่ **ไม่ได้ส่ง `unknownByReason`**
                                ⇒ จอที่อ่านแต่ช่องนับ เลยเงียบสนิททั้งที่มี 4 รหัสยืนยันไม่ได้
                                ⇒ ตอนนี้นับเองจากแถวเมื่อไม่มีช่องนับ **และเขียนกำกับว่านับมาจากไหน**
                                   (CLAUDE.md ข้อ 4: เลขจากคนละแหล่ง ห้ามวางคู่กันเฉย ๆ)
                             ② เหตุที่จอไม่รู้จัก เคยถูกเหมารวมว่า "คลังไม่รู้จักรหัสนี้เลย"
                                ⇒ วันที่ยิงจริงได้ `no_full_plan` (ท่อไม่ได้ส่งแผนเต็ม) จอจะประกาศว่า
                                   **คลังไม่รู้จัก ทั้งที่ไม่เกี่ยวกับคลังเลยสักนิด**
                                ⇒ เหตุที่ไม่อยู่ในตาราง ต้องพูดว่า **"จอยังไม่รู้จักเหตุนี้"**
                                   ห้ามเดาความหมายให้ — เดาผิดคือส่งคนไปนับของที่ไม่มีปัญหา */
                          const จากช่องนับ = Object.entries(v.unknownByReason ?? {})
                          const นับเอง = !จากช่องนับ.length && (v.unknown ?? []).length > 0
                          const แยกเหตุ = นับเอง
                            ? Object.entries(
                                (v.unknown ?? []).reduce<Record<string, number>>((a, u) => {
                                  const k = u.reason || 'ไม่บอกเหตุ'
                                  a[k] = (a[k] ?? 0) + 1
                                  return a
                                }, {}),
                              )
                            : จากช่องนับ
                          /* เหตุที่จออธิบายได้ · ระดับติดมากับตัวเหตุ ไม่ใช่ "ทุกตัวที่ไม่ใช่ X คือควรตรวจ"
                             ⚠️ ของที่ไม่อยู่ในตารางนี้ **ห้ามเดาว่าอยู่ขั้วไหน** */
                          const เหตุที่รู้จัก: Record<string, { คำ: string; ระดับ: 'ปกติ' | 'ควรตรวจ' }> = {
                            segment_of_cut_product: { คำ: 'รหัสท่อนของสินค้าแบบตัด (คลังมีรหัสแม่) — ปกติ', ระดับ: 'ปกติ' },
                            /* ท่อแยกออกมาให้ 19 ก.ย. 2569 (0bbe46c) — คลัง **รู้จักดี** แต่ตัวเทียบ
                               คิดจำนวนชุดหลายชิ้นไม่ได้ (โค้ดข้ามเองเมื่อ count !== 1)
                               ⇒ ขึ้นเทา ไม่ใช่เหลือง · ทาเหลืองคือส่งคนไปตามของที่ไม่ได้หาย */
                            bundle_multi_part: { คำ: 'ชุดหลายชิ้น — คลังรู้จัก แต่ตัวเทียบยังคิดจำนวนไม่ได้', ระดับ: 'ปกติ' },
                            no_full_plan: { คำ: 'ท่อไม่ได้ส่งแผนเต็มมา — ไม่เกี่ยวกับตัวสินค้า', ระดับ: 'ปกติ' },
                            not_in_warehouse: { คำ: 'คลังไม่มีรหัสนี้เลย — ควรตรวจ', ระดับ: 'ควรตรวจ' },
                          }
                          const รวม = แยกเหตุ.reduce((a, [, n]) => a + n, 0)
                          const ควรตรวจ = แยกเหตุ.reduce((a, [k, n]) => a + (เหตุที่รู้จัก[k]?.ระดับ === 'ควรตรวจ' ? n : 0), 0)
                          const ยังไม่รู้จัก = แยกเหตุ.filter(([k]) => !เหตุที่รู้จัก[k])
                          return (
                            <>
                              {แยกเหตุ.length > 0 && (
                                <span className="block mb-0.5">
                                  <span className="text-gray-600">
                                    ยังยืนยันไม่ได้ {รวม} รหัส —{' '}
                                    {แยกเหตุ.map(([k, n], i) => (
                                      <span key={k} className={เหตุที่รู้จัก[k]?.ระดับ === 'ปกติ' ? 'text-gray-600' : 'text-amber-800'}>
                                        {i ? ' · ' : ''}
                                        {เหตุที่รู้จัก[k]?.คำ ?? `${k} — จอยังไม่รู้จักเหตุนี้`} <b>{n}</b>
                                      </span>
                                    ))}
                                  </span>
                                  {นับเอง && (
                                    <span className="block text-gray-400">
                                      (จอนับเองจากแถว `unknown` เพราะรอบนี้ท่อไม่ได้ส่งช่องนับ `unknownByReason` มา)
                                    </span>
                                  )}
                                  {/* ⚠️ ตัวที่ควรตรวจต้องแยกออกมาพูดซ้ำ ไม่ใช่ปนอยู่ในแถวเดียวกับของปกติ */}
                                  {ควรตรวจ > 0 && (
                                    <span className="block text-amber-800">
                                      ⚠️ ใน {รวม} รหัสนั้น มี <b>{ควรตรวจ}</b> รหัสที่คลังไม่รู้จักเลย ⇒ <b>ควรตรวจ</b>
                                    </span>
                                  )}
                                  {ยังไม่รู้จัก.length > 0 && (
                                    <span className="block text-amber-800">
                                      ⚠️ มีเหตุที่จอนี้ <b>ยังไม่รู้จัก</b> {ยังไม่รู้จัก.length} แบบ —{' '}
                                      {ยังไม่รู้จัก.map(([k, n]) => `${k} (${n})`).join(' · ')}
                                      {' '}⇒ <b>ตัดสินไม่ได้ว่าปกติหรือควรตรวจ</b> ต้องถามฝั่งท่อว่าหมายถึงอะไร
                                    </span>
                                  )}
                                </span>
                              )}
                          ...
                          return ส่งไป > ตอบกลับ ? (
                            <span className="block text-amber-800 mb-0.5">
                              ...
                              {' '}⇒ อีก {ส่งไป - ตอบกลับ} รหัส <b>ตัดสินไม่ได้</b> (ท่ออาจจัดไว้กองอื่น เช่นรหัสที่ไม่มีในทะเบียน)
                        ...
                        {/* ตอน inconclusive ประโยคนี้ถูกยกขึ้นไปพูดข้างบนแล้ว ⇒ ไม่ต้องซ้ำ */}
                        {v.note && !v.inconclusive && <span className="block text-gray-400 mt-0.5">{v.note}</span>}
                      </div>
                    )
                  )}
                </div>
