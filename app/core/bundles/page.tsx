...
interface Resp {
   ...
   *     ก่อนหน้านั้นเป็นงานเดียวกัน ⇒ สองเวลานี้เคยห่างกัน **ไม่กี่วินาที** ซึ่งเป็นเบาะแสที่ทำให้จับได้
   *     (วัดได้ 2 ครั้ง: 8 วินาที และ 4 วินาที — **ไม่ใช่ค่าคงที่** อย่าเอาเลขใดเลขหนึ่งไปทำเกณฑ์)
   *     🔑 ตอนแรกผมเขียนตรงนี้ว่า "8 วินาทีเสมอ" ซึ่งผิดสองชั้น: วัดแค่สองครั้ง และสองครั้งนั้นไม่เท่ากัน
   *        ⇒ ตัวอย่างของการเอาตัวอย่างไม่กี่ตัวมาเขียนเป็นกฎ แล้วคนอ่านรอบหน้าจะเชื่อว่าพิสูจน์แล้ว
    ...
    <div className="p-4 md:p-6">
      ...
      {data && !data.skip && (
        <>
          ...
          {/* 🔴 **คอมเมนต์ตรงนี้เคยเขียนว่า "⇒ ถอดออก" ซึ่งไม่จริง** (แก้ 19 ก.ย. 2569 · ใบ S4)
              แท็บยัง **เรนเดอร์อยู่จริง** ที่บล็อก `<Tabs>` ข้างบนในไฟล์เดียวกันนี้
              และเปิดจอจริงบน production ดูแล้วเห็นครบ: ทั้งหมด (360) · เปิดใช้งาน (360) · ปิดใช้งาน (0)
              ⇒ คอมเมนต์กับโค้ดขัดกันเองในไฟล์เดียว · คนอ่านรอบหน้าจะเชื่อคอมเมนต์
                 แล้วไปตามหาโค้ดที่ "ถอดออกแล้ว" ซึ่งไม่มีอยู่ หรือแย่กว่านั้นคือถอดของที่ยังใช้อยู่ทิ้ง
              🔑 บทเรียนเดียวกับทั้งใบ S4: **ข้อความที่เล่าสภาพ ต้องผูกกับของจริง หรือไม่ก็อย่าเขียน**

              ⚠️ ของที่ยังไม่ตัดสิน และ **ต้องให้เจ้าของร้านตัดสิน ไม่ใช่เราถอดเอง**:
                 ZORT ไม่มีแท็บในจอนี้ (หลังแถวค้นหาคือหัวตารางเลย) แต่กฎคือ "เหมือน ZORT 100%"
                 ⇒ การมีแท็บคือส่วนต่างที่จงใจหรือหลงเหลือ — ยังไม่มีใครตัดสิน
                 🚫 ห้ามถอดเองเพราะอ่านคอมเมนต์เก่าแล้วคิดว่าเคยตกลงกันไว้ (ตัวกรองฝั่งท่อคือ `only=active|inactive`) */}
