      ...
      {data && (
        <>
          ...
          <div className="flex flex-wrap items-center gap-2 mt-3 text-[12.5px] text-gray-600">
            ...
            />
            {/* 🔴 **ชนิดเอกสารที่ท่อใช้จริง** (เพิ่ม 19 ก.ย. 2569)
                📌 ไม่ต้องใช้ช่องรับรอง — การเทียบตรงนี้อยู่บรรทัดเดียวกับ `applied.type` ⇒ ด่านเห็นเอง
                   (ช่องรับรองมีไว้สำหรับท่าที่ตัวจับมองไม่เห็นเท่านั้น · ใส่เกินจำเป็น = ลดน้ำหนักของคำรับรองทั้งระบบ)
                จอนี้เทียบ `page` มาตลอด แต่ **ไม่เคยเทียบ `type`** ทั้งที่ส่งไปทุกครั้งที่เลือกชนิด
                ⇒ วันที่ท่อเมินชนิด คนเลือก "ใบหัก ณ ที่จ่าย" แล้วได้เอกสารทุกชนิด โดยไม่มีอะไรฟ้อง
                ⚠️ **ยิงวัดแล้ว 19 ก.ย. 2569**: ท่อกรองจริง (type=1 ⇒ applied.type=1 "ใบเสร็จรับเงิน")
                   และค่าที่ท่อไม่รู้จัก (type=99) ⇒ **ท่อคืน `applied` เป็น null ทั้งก้อน**
                   ⇒ จึงต้องแยก "ท่อไม่ได้บอก" ออกจาก "ท่อใช้ชนิดอื่น" ไม่ยุบเป็นข้อความเดียว */}
            {type !== '' && data.applied != null && String(data.applied.type ?? '') !== type && (
              <span className="text-amber-800">
                ⚠️ จอขอชนิด <b>{type}</b> แต่ท่อใช้{' '}
                <b>{data.applied.typeLabel ?? (data.applied.type == null ? 'ทุกชนิด' : String(data.applied.type))}</b>
                {' '}— แถวที่เห็นไม่ได้กรองตามที่เลือก
              </span>
            )}
            {type !== '' && data.applied == null && (
              <span className="text-amber-800">
                ⚠️ ท่อ<b>ไม่ได้บอกว่าใช้ชนิดไหน</b> (ชนิด <b>{type}</b> อาจไม่ใช่ค่าที่ท่อรู้จัก) — อย่าเชื่อว่ากรองแล้ว
              </span>
            )}
