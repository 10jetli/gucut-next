  ...
  return (
    <div className="p-4 md:p-6">
      ...
      <p className="text-[11px] text-gray-400 mt-3 leading-relaxed">
        ...
        แต่ละรายการชี้ไฟล์ต้นทางไว้ให้ตรวจย้อนได้
      </p>

      {/* ── ของ ZORT ในจอเดียวกัน — **กดอ่านจากจอจริง 16 ก.ย. 2569** ไม่ได้จำมาจากภาพ ──
          แท็บช่องทางของเขา: โปรแกรม · อีเมล · แอปพลิเคชันมือถือ
          เรื่องที่แจ้ง 7 เรื่อง + ตัวเลือกผู้รับ 5 แบบ (ทั้งหมด · ตามสิทธิ์การใช้งาน · กำหนดเอง ·
          ค่าเริ่มต้น (Admin) · ปิดการแจ้งเตือน)
          🔴 เขียนเป็นตารางเทียบ **เพราะคนที่ชิน ZORT จะหาของพวกนี้** ถ้าไม่บอกจะคิดว่าเราลืม
             และต้องบอกด้วยว่าอันไหน "ยังไม่ได้ทำ" อันไหน "ทำไม่ได้เพราะต้นทางไม่มีข้อมูล" */}
      <div className="bg-white border border-gray-200 rounded-md p-4 mt-4">
        <p className="text-[13.5px] font-semibold text-gray-800 mb-1">📋 แจ้งเตือนที่ ZORT มีในจอนี้ — ของเราอยู่ตรงไหน</p>
        <p className="text-[11.5px] text-gray-500 mb-2.5">
          อ่านจากจอ ZORT จริง 16 ก.ย. 2569 · ช่องทางของเขาคือ <b>โปรแกรม · อีเมล · แอปพลิเคชันมือถือ</b>
          {' '}(ของเราคือ Telegram · Web Push · LINE) · เขาตั้งผู้รับได้ 5 แบบ
          {' '}(ทั้งหมด · ตามสิทธิ์การใช้งาน · กำหนดเอง · ค่าเริ่มต้น (Admin) · ปิดการแจ้งเตือน)
          {' '}— <b>ของเราตั้งที่ env/โค้ด ไม่มีปุ่มเลือกผู้รับ</b>
        </p>
        <div className="overflow-x-auto">
          <table className="w-full min-w-[520px] text-[12.5px]">
            <thead className="border-b border-gray-200">
              <tr>
                <th className="text-left font-normal text-[11.5px] text-gray-500 px-2 py-1.5">เรื่องที่ ZORT แจ้ง</th>
                <th className="text-left font-normal text-[11.5px] text-gray-500 px-2 py-1.5">ของเรา</th>
              </tr>
            </thead>
            <tbody>
              {[
                ['ยอดขายประจำวัน', 'ไม่ได้ส่งสรุปตามเวลา — ดูได้ตลอดที่ รายงาน → ยอดขาย (ยังไม่ได้ทำ ไม่ใช่ทำไม่ได้)'],
                ['ยอดขายประจำเดือน', 'เหมือนข้างบน — จอรายงานมีมุมมองรายเดือนอยู่แล้ว'],
                ['สินค้าใกล้หมด', 'ทำไม่ได้แบบเขา — ZORT คิดจากกลุ่ม Lead Time ซึ่งร้านยังไม่ได้ตั้งสักกลุ่ม (ยืนยันสด 16 ก.ย.)'],
                ['สินค้าหมดแล้ว', 'มีตัวเลขบนจอ (รายงาน → สินค้า) แต่ยังไม่ส่งแจ้งเตือน'],
                ['แจ้งการชำระเงิน', 'มีฝั่งลูกค้า (ทวงยอดค้างทาง LINE/Push) — ยังไม่มีแจ้งทีมเมื่อเงินเข้า'],
                ['มีลูกค้าที่สินค้าใกล้หมด', 'ยังไม่มี — ต้องรู้ "ใกล้หมด" ก่อน (ติดข้อเดียวกับด้านบน)'],
                ['อนุมัติใบเสนอราคา', 'ยังไม่มี — จอใบเสนอราคาอ่านอย่างเดียว ยังไม่มีขั้นอนุมัติในระบบเรา'],
              ].map(([zort, ours]) => (
                <tr key={zort} className="border-b border-gray-100 last:border-0">
                  <td className="px-2 py-1.5 text-gray-800 align-top whitespace-nowrap">{zort}</td>
                  <td className="px-2 py-1.5 text-gray-600 leading-relaxed">{ours}</td>
                </tr>
              ))}
            </tbody>
          </table>
