...
//
// ฝั่งเซิร์ฟเวอร์สำรอง Netlify Blobs → Cloudflare D1 (คนละบริษัท คนละคีย์) **ทุก 6 ชั่วโมง**
// (ลดจากทุกชั่วโมง 18 ก.ย. 2569 — ทุกรอบได้ saved 0 unchanged 52 ⇒ ดึงทั้งกองแล้วไม่มีอะไรเปลี่ยน)
...
interface StoreRow {
  ...
  /** คีย์ที่ **ต้นทางลบ** ใน 24 ชม.ล่าสุด และช่วง 24–48 ชม.ก่อนหน้า (นับแบบวันไทย · ท่อส่งตั้งแต่ 18 ก.ย. 2569)
   *  🔑 ท่อตั้งใจส่งสองก้อนมาให้เทียบ **แต่ไม่ตัดสินให้ว่าเท่าไรคือพุ่ง** ⇒ เกณฑ์อยู่ฝั่งจอ
   *  ⚠️ ไม่มีช่องนี้ = ท่อรุ่นเก่า ⇒ ต้องเขียนว่า "ยังเทียบไม่ได้" ห้ามแปลว่า "ไม่มีอะไรผิดปกติ" */
  gone24h?: number; gonePrev24h?: number
...
export default function BackupPage() {
  ...
  /* ── "พุ่งขึ้นผิดปกติ" คิดยังไง ────────────────────────────────────────
     จอนี้เขียนเตือนตัวเองมานานแล้วว่า "ถ้าเลขนี้พุ่งขึ้นผิดปกติ แปลว่ามีอะไรกำลังลบข้อมูลอยู่ ควรมาดู"
     🔴 แต่เดิม **ไม่มีโค้ดบังคับเลย** — เป็นหน้าที่ของคนที่ต้องจำเลขเมื่อวานเอง (ซึ่งไม่มีใครจำ)
     ⇒ ขอให้ท่อส่งยอด 24 ชม. กับ 24–48 ชม.ก่อนหน้ามาคู่กัน (gucut-web eda0b0d) แล้วเทียบตรงนี้

     เกณฑ์ที่ตั้ง (เลขของเราเอง ปรับได้ที่นี่ที่เดียว):
     · ต้องมากกว่า 20 คีย์ก่อน — ไม่งั้น 1 → 3 คีย์ก็เด้ง แล้วคนจะเลิกอ่าน
     · และต้องมากกว่าช่วงก่อนหน้า 3 เท่า (หรือช่วงก่อนหน้าเป็น 0 = ไม่เคยลบ แล้วอยู่ ๆ ลบทีเดียวเยอะ)
     ⚠️ ท่อรุ่นเก่าไม่ส่งช่องนี้ ⇒ `null` = **ยังเทียบไม่ได้** ห้ามแปลว่าปกติ */
  const ก้อนลบ = (() => {
    const มีช่อง = stores.some((x) => typeof x.gone24h === 'number')
    if (!มีช่อง) return null
    const วันนี้ = stores.reduce((a, x) => a + (Number(x.gone24h) || 0), 0)
    const ก่อนหน้า = stores.reduce((a, x) => a + (Number(x.gonePrev24h) || 0), 0)
    const พุ่ง = วันนี้ > 20 && (ก่อนหน้า === 0 ? true : วันนี้ >= ก่อนหน้า * 3)
    return { วันนี้, ก่อนหน้า, พุ่ง }
  })()
  const mins = minutesSince(st?.lastRun)
  /* สำรองทุก **6 ชั่วโมง** (ฝั่งท่อลดรอบ 18 ก.ย. 2569 · cron นาทีที่ 40 ทุก 6 ชม.) — เดิมทุกชั่วโมง เกณฑ์เดิม 125 นาที
     ⚠️ ห้ามเขียน cron แบบมีเครื่องหมายทับกับดอกจันติดกันในคอมเมนต์บล็อก — มันปิดคอมเมนต์กลางประโยค
        (เพิ่งโดนกับตัวเองตอนเขียนบรรทัดนี้ · TypeScript ฟ้อง TS1109 ซึ่งอ่านไม่ออกว่าเกิดจากอะไร)
     🔴 **เกณฑ์นี้ผูกกับตาราง cron ของท่อโดยตรง** — ลดรอบแล้วไม่แก้เกณฑ์ = จอขึ้นเหลืองทุกวันทั้งที่ปกติ
        แล้วคนจะเลิกอ่านจอนี้ ซึ่งแย่กว่าไม่มีจอเลย
     ⇒ ตั้งที่ 12 ชั่วโมง = เผื่อพลาดได้ 1 รอบเต็ม ๆ ก่อนจะร้อง (ท่าเดียวกับที่ฝั่งท่อตั้งไว้) */
  const stale = mins !== null && mins > 12 * 60
  ...
  return (
    <div className="p-4 md:p-6">
      <PageHead
        title="สำรองข้อมูล"
        summary="สำเนาของข้อมูลในเว็บ เก็บไว้คนละบริษัทและคนละคีย์กับตัวจริง · ทำเองทุก 6 ชั่วโมง"
        actions={
          <>
          ...
          <p className="text-[12px] text-gray-500 mt-2 leading-relaxed">
            ...
            {ก้อนลบ === null ? (
              <span className="text-gray-400">
                ⏳ <b>ยังเทียบกับช่วงก่อนหน้าไม่ได้</b> (ท่อรุ่นนี้ยังไม่ส่งยอดรายช่วงมา) —
                ไม่ได้แปลว่าไม่มีอะไรผิดปกติ แปลว่ายังไม่รู้
              </span>
            ) : ก้อนลบ.พุ่ง ? (
              <span className="block text-red-800 bg-red-50 border border-red-300 rounded px-2.5 py-1.5 my-1 font-semibold">
                🔴 24 ชม.ล่าสุด ต้นทางลบไป <b>{num(ก้อนลบ.วันนี้)}</b> คีย์ ·
                ช่วง 24–48 ชม.ก่อนหน้าลบ <b>{num(ก้อนลบ.ก่อนหน้า)}</b> ⇒ <b>เพิ่มขึ้นผิดปกติ</b>
                <span className="block font-normal">
                  สำเนายังอยู่ครบ (เราไม่ลบตาม) แต่ควรไปดูว่าอะไรกำลังลบของที่ต้นทาง
                </span>
              </span>
            ) : (
              <span className="text-gray-500">
                24 ชม.ล่าสุดต้นทางลบ <b>{num(ก้อนลบ.วันนี้)}</b> คีย์ · ช่วงก่อนหน้า <b>{num(ก้อนลบ.ก่อนหน้า)}</b> ⇒ ยังอยู่ในระดับปกติ
                <span className="text-gray-400"> (เส้นที่ใช้: เกิน 20 คีย์ และมากกว่าช่วงก่อน 3 เท่า — เกณฑ์ของเราเอง)</span>
              </span>
            )}
            <br />
