...
interface Resp {
   ...
   *  ⚠️ ค่านี้เป็น **UTC** (`datetime('now')` ของ D1) และ **ชื่อช่องไม่ได้บอกไว้** ⇒ เคยถูกอ่านผิดเป็นวันไทยมาแล้ว */
  recipeAt?: string
  /** วันไทยสำเร็จรูปจากท่อ — **ถ้ามีต้องใช้ตัวนี้ก่อนเสมอ** (ท่อเป็นฝ่ายรู้เขตเวลาของค่าตัวเอง ไม่ใช่ให้จอเดา)
   *  ฝั่งท่อเริ่มส่งที่เส้น `shopeeunlisted` แล้ว (gucut-web a482ef7) · เส้นนี้ยังไม่ส่ง ⇒ จอแปลงเองไปพลางก่อน
   *  เมื่อท่อส่งมาเมื่อไหร่ จอจะสลับมาใช้ค่าของท่อเองโดยไม่ต้องแก้โค้ดอีก */
          ...
          <Card padded={false} className="overflow-hidden">
            ...
            {view === 'buildable' && (
              <p className="text-[12px] text-amber-800 bg-amber-50 border-t border-amber-100 px-4 py-3 leading-relaxed">
                ...
                {/* ⚠️ ประตูต้องถาม **ทั้งสองช่อง** — ถ้าวันหลังท่อส่งแต่ `recipeDayTH` แล้วเลิกส่ง `recipeAt`
                    บรรทัดนี้จะหายไปเงียบ ๆ ทั้งที่ข้อมูลมีอยู่ (บั๊กคลาสเดียวกับที่แก้ไปแล้วที่จอมาร์เก็ตเพลส fc2470c) */}
                {(data.recipeDayTH || data.recipeAt) && (
