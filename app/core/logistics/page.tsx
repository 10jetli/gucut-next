...
interface Resp {
  ...
  /** 🔑 ท่อประกาศ **ขอบเขตของตัวนับแต่ละตัว** มาเอง (gucut-web 571db11 · 19 ก.ย. 2569)
   *  เกิดเพราะฝั่งท่อกำลังจะติดป้ายกลางว่า `total = ทั้งชุด` แล้ววัดจริงพบว่า
   *  `total` ขยับตาม `carrier` แต่ **ไม่ขยับตาม `only`** ⇒ ความหมายขึ้นกับว่าเส้นนั้นกรองตรงไหน
   *  ⇒ ทะเบียนกลางเลย **ปฏิเสธที่จะติดป้าย** แล้วให้แต่ละเส้นประกาศเอง
   *  ⚠️ จอต้อง **แสดงประโยคของท่อ ไม่ใช่พิมพ์คำอธิบายของตัวเองคู่ขนาน**
   *     ไม่งั้นวันที่ท่อเปลี่ยนวิธีนับ คำบนจอจะค้างเป็นเท็จเงียบ ๆ (คลาสที่ทีมไล่ปิดทั้งวัน) */
  'ขอบเขตตัวนับ'?: Record<string, string>
  ...
  return (
    <div className="p-4 md:p-6">
          ...
          data
            ? (
              <>
                ...
                {/* 🔴 **เลขหัวจอกับแถวที่เห็น คนละกติกาเมื่อเลือกแท็บ** — ท่อบอกเองว่า
                    `total` นับ **ก่อน** `only` ⇒ กดแท็บ "ยังไม่มีเลขพัสดุ" แล้วเลขหัวจอไม่ขยับ
                    คนอ่านจะนึกว่าแท็บไม่ทำงาน หรือแย่กว่านั้นคือเชื่อว่าเลขนี้คือจำนวนของแท็บ
                    ⇒ พูดออกมาตรง ๆ ตอนที่ต่างกันจริง · ใช้ประโยคของท่อ ไม่พิมพ์เอง */}
                {only && (
                  <span className="text-amber-800">
                    {' '}(ยังไม่รวมตัวกรองแท็บ
                    {data['ขอบเขตตัวนับ']?.['🔑 ใช้ตัวไหน']
                      ? ` — ${data['ขอบเขตตัวนับ']!['🔑 ใช้ตัวไหน']}`
                      : ' — จำนวนของแท็บที่เลือกอยู่ดูที่เลขหน้าด้านล่าง'})
                  </span>
                )}
      ...
      <p className="text-[12.5px] text-amber-900 bg-amber-50 border border-amber-200 rounded px-3 py-2 mb-3">
        ...
        โดยรายการล่าสุดที่เห็นคือ 17 ก.ย. 2569 · อ่านจากจอเขาเมื่อ 18 ก.ย. 2569)
      </p>
      ...
      {data && (
        <>
          {/* 🔴 กล่องนี้คือหัวใจของจอนี้ — ห้ามถอด
              ...
              ✅ **ข้อความ "ยังไม่รู้กติกาของ ZORT" ตรงนี้หมดอายุแล้ว — ตอบได้ 18 ก.ย. 2569**
                 (พิสูจน์ไว้ในคอมเมนต์ถัดลงไปในไฟล์นี้: จอเขาเป็นจอจองขนส่ง · สุ่มเลขพัสดุ 3 ใบ เจอครบ 3/3)
                 ⚠️ เก็บบรรทัดนี้ไว้เพราะ **คอมเมนต์ที่ค้างว่า "ยังไม่รู้" ในไฟล์เดียวกับคำตอบ
                    จะทำให้คนอ่านรอบหน้าไปตามหาคำตอบที่มีอยู่แล้ว** และอาจไปเขียนคำเดิมทับของใหม่
                 (เจอตอนกวาดคำเตือนหมดอายุ 18 ก.ย. 2569 — คลาสเดียวกับป้าย "ท่อนท้ายยังไม่ได้แก้" ที่ถอดไปแล้ว) */}
          <div className="text-[12.5px] text-amber-800 bg-amber-50 border border-amber-200 rounded-md px-3.5 py-2.5 mb-3 leading-relaxed">
            ...
            {typeof data.zortShows === 'number' && (
              <>
                <br />
                {/* ✅ **ตอบได้แล้ว 18 ก.ย. 2569 — เดิมจอเขียนว่า "ยังไม่รู้ว่า ZORT คัดด้วยเงื่อนไขอะไร"**
                    เปิดจอ ZORT อ่านจริง: `/Logistics/list` ของเขาเป็น **จอจองขนส่ง** (พิมพ์ฉลาก · เลือกน้ำหนัก/กล่อง · เพิ่มผู้รับ)
                    ⇒ ประชากรคือ "พัสดุที่ร้านจองผ่าน ZORT เอง" · พิสูจน์ว่าเป็นชุดย่อยของเราจริง
                       ด้วยการสุ่มเลขพัสดุจากจอเขา 3 ใบ มาค้นในคลังเรา — เจอครบ 3/3 (ใบ SO-2026090xx) */}
                · <b>ของ ZORT</b> จอ &ldquo;บริการขนส่ง&rdquo; แสดง <b>{fmtNum(data.zortShows)}</b> รายการ
                {' '}— <b>คือพัสดุที่ร้านจองผ่าน ZORT เอง</b> (L Shipping Point) ไม่ใช่ทุกใบที่ส่งของ
                {' '}· พิสูจน์แล้ว 18 ก.ย. 2569: สุ่มเลขพัสดุจากจอเขา 3 ใบ ค้นในคลังเรา<b>เจอครบ 3/3</b>
                {' '}⇒ ของเขาเป็น<b>ชุดย่อยของของเรา</b> ส่วนที่เกินมาคือใบมาร์เก็ตเพลสที่แพลตฟอร์มส่งเอง
              </>
            )}
            <br />
            ⇒ ใช้จอนี้ดู<b>ว่าใบไหนส่งด้วยเจ้าไหน</b>ได้ แต่<b>ห้ามเอาจำนวนไปเทียบกับ ZORT</b> ตรง ๆ
            {/* ข้อความ `note` มาจากฝั่งท่อ — โชว์ดิบ ๆ ให้รู้ว่าใครพูด
                ✅ **ป้าย "ท่อนท้ายยังไม่ได้แก้" ถอดออกแล้ว 18 ก.ย. 2569**: ยิงอ่าน note ของจริงวันนี้
                   ท่อแก้ให้ถูกแล้ว (เขียนว่า "คนละหน่วยนับ ⇒ เลขของเรามากกว่ามาก")
                   ⇒ ป้ายเตือนที่ค้างไว้กลายเป็น **ตัวมันเองที่โกหก** — บอกคนอ่านว่าข้อความอีกฝั่งยังผิดทั้งที่แก้แล้ว */}
            {data.note && (
              <>
                <br />
                <span className="text-amber-900/70">
                  <b>ข้อความจากท่อ:</b> {data.note}
                </span>
              </>
            )}
          </div>
          ...
          {/* 🔴 **"เก็บเงินปลายทาง" เป็นตัวกรองคนละแกนกับสองแท็บแรก** (วัดของจริง 18 ก.ย. 2569)
              ยังไม่มีเลขพัสดุ 1,181 + ส่งแล้ว 34,583 = 35,764 = ทั้งหมด **พอดีเป๊ะ**
              ⇒ สองแท็บนั้นแบ่งกองกันครบ · ส่วน COD 28,880 **ซ้อนทับทั้งสองกอง**
              ⚠️ วางเรียงกันในแถบเดียวโดยไม่บอก = คนบวกทั้งสี่แล้วได้ 64,644 แล้วคิดว่าตัวเลขพัง */}
          {(typeof data?.cod === 'number' && data.cod > 0) && (
            <p className="text-[11.5px] text-gray-500 mt-1">
              ℹ️ <b>เก็บเงินปลายทาง</b> เป็นตัวกรอง<b>คนละแกน</b> — ซ้อนกับ &ldquo;ยังไม่มีเลขพัสดุ&rdquo; และ &ldquo;ส่งแล้ว&rdquo;
              ⇒ <b>อย่าเอาตัวเลขทั้งสี่แท็บมาบวกกัน</b> (สองแท็บแรกรวมกันได้เท่ากับทั้งหมดอยู่แล้ว)
            </p>
          )}
