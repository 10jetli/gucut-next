...
//
// ✅ **ยิงพิสูจน์แล้ว 18 ก.ย. 2569: เส้น `?transfer=` รับเฉพาะ `id` ตัวเลขของ ZORT**
//    ส่งเลขที่ใบ (`TF-202609004`) ⇒ ตอบ `{"ok":false,"badId":true}` พร้อมข้อความว่าต้องเป็นตัวเลข
//    ⇒ คนหน้าคลังพิมพ์ **เลขที่ใบ** ตามกระดาษเสมอ ไม่มีใครรู้ id ตัวเลข
//    ⇒ จอจึงหาใบในกระจกก่อน แล้ว **ยิงสดซ้ำด้วย id ที่กระจกถืออยู่** เพื่อให้ได้บรรทัดสินค้ามาเติม
//       (ไม่งั้นพิมพ์เลขใบแล้วจะได้แต่หัวใบ ทั้งที่ id อยู่ในมือเราอยู่แล้ว)
// ⚠️ **ค้นด้วยเลขพัสดุยังไม่ยืนยัน** — จอ ZORT เขียนว่า "กรอกหมายเลขรายการ หรือ Tracking No."
//    (อ่านจากจอจริง 18 ก.ย. 2569) แต่ฝั่งเรายังไม่มีใครยิงพิสูจน์ว่ากระจกค้นด้วยเลขพัสดุเจอไหม
//    ⇒ **ห้ามเขียนบนจอว่าค้นได้** · ให้ลองแล้วถ้าไม่เจอค่อยบอกว่าเป็นไปได้สามอย่าง
//    (พิมพ์ผิด / เป็นเลขพัสดุ / ใบยังไม่เข้ากระจก)
...
export default function ReceivePage() {
  ...
  const find = useCallback(async () => {
    ...
    try {
      ...
      /* 🔁 **เจอใบในกระจกแล้ว ⇒ ลองดึงสดอีกครั้งด้วย `id` ที่กระจกถืออยู่** (เพิ่ม 18 ก.ย. 2569)
         ที่มา: ยิงพิสูจน์เองวันนี้ว่าเส้น `?transfer=` **รับเฉพาะ id ตัวเลขของ ZORT**
         (ส่งเลขที่ใบ `TF-202609004` ⇒ ตอบ `badId` พร้อมข้อความว่าต้องเป็นตัวเลข)
         ⇒ คนที่ยืนอยู่หน้าคลังพิมพ์ **เลขที่ใบ** ตามที่เห็นบนกระดาษเสมอ ไม่มีใครรู้ id ตัวเลข
            ⇒ ขา ① พลาดทุกครั้งที่พิมพ์เลขใบ แล้วจอตกมาทางกระจกซึ่ง **ไม่มีบรรทัดสินค้า**
            ทั้งที่ id อยู่ในมือเราอยู่แล้วจากผลค้นกระจก
         ⇒ ยิงซ้ำด้วย id ⇒ ได้บรรทัดสินค้า + เลขพัสดุมาเติมให้เหมือนตอนพิมพ์ id เอง
         ⚠️ ล้มเหลวก็ไม่เป็นไร — ถอยไปใช้หัวใบจากกระจกเหมือนเดิม และ **ต้องยังบอกว่ามาจากกระจก**
            (จอ ZORT ให้กรอก "หมายเลขรายการ หรือ Tracking No." ⇒ คนคาดหวังว่าพิมพ์เลขใบแล้วต้องได้ของ) */
      const เลขไอดี = String(rows[0]?.id ?? '').trim()
      if (/^\d+$/.test(เลขไอดี)) {
        const สด: Detail = await fetch(`/api/web/core?transfer=${encodeURIComponent(เลขไอดี)}`)
          .then((x) => x.json()).catch(() => ({ error: 'เรียกไม่สำเร็จ' }))
        if (สด && !สด.error && สด.number) {
          setDetail(สด); setSrc('live')
          setDoc({ number: สด.number, status: สด.status, from_wh: สด.from, to_wh: สด.to, transfer_date: สด.date })
          if (Array.isArray(สด.lines) && สด.lines.length) {
            setLines(สด.lines.map((l) => ({ sku: String(l.sku ?? ''), qty: String(l.qty ?? '') })))
          } else {
            setLines([{ sku: '', qty: '' }])
          }
          setStep(1)
