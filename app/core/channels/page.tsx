 ...
 *  ✅ **สลับแล้ว 5 ก.ย. 2569 21:0x** หลังยิงของจริงเทียบทีละช่องจนครบ
 *     (ร้าน 2 ร้านตรงทุกช่อง · ช่องทางครบ 8 ตัว · ยอดรายช่องทางตรง · ป้ายขอบเขตตรง)
 *
 *  🔴 **เหตุผลที่สลับเปลี่ยนไปจากตอนวางแผน — อ่านก่อนคิดจะย้อนกลับ**
 *     ตอนบ่ายเส้นเดิมใช้ 4.2 วิ จึงสลับ "เพื่อให้เร็วขึ้น"
 *     แต่ฝั่งท่อแก้ให้ยิง D1 พร้อมกันไปแล้วในรอบเดียวกัน ⇒ วัดจริงหลัง deploy:
 *       orderfacets 1.09-1.69 วิ · list=orders 0.95-1.73 วิ ⇒ **เร็วเท่ากัน วัดไม่ออก**
 *     ที่ยังต่างกันคือ **งานที่ฐานข้อมูลต้องทำ** (อ่านจากหัว x-d1-*):
 *       orderfacets = **2 รอบ · 614 ms** · list=orders = **11 รอบ · 3,922 ms**
 *     ⇒ เหตุผลที่สลับตอนนี้คือ **ลดงานฐาน 6 เท่า** ไม่ใช่ "จอเร็วขึ้น"
 *     การยิงพร้อมกัน **ซ่อน** งานนั้นไว้ ไม่ได้ทำให้มันหายไป — คนใช้พร้อมกันหลายคนเมื่อไหร่
 *     ภาระ 11 รอบต่อคนจะโผล่กลับมาเป็นความช้าที่หาสาเหตุไม่เจอ
...
const RECENT_QS = `/api/web/core?list=orderfacets&from=${thaiDay(30)}&to=${thaiDay(0)}`
    ...
    <div className="p-4 md:p-6 space-y-4 md:space-y-5">
      ...
      {st && !error && (
        <>
          ...
          <Card>
            {/* 🔴 **การ์ดนี้กับการ์ด "ท่อดึงตรง" ข้างล่าง นับคนละกอง** (เจอตอนกวาดจริง 14 ก.ย. 2569)
                บนจอเดียวกันจะเห็น "Shopee 1 ใบ · 1,000" แล้วอ่านต่ออีกสองนิ้วเจอ
                "Shopee — ยังไม่เห็นออเดอร์จากท่อนี้ในฐานของเรา" ⇒ ดูเหมือนขัดกันเอง
                ความจริง: อันบนนับจาก **กระจกออเดอร์ (ที่มาจาก ZORT)** · อันล่างนับเฉพาะ
                **ออเดอร์ที่ท่อดึงตรงจาก API ของแพลตฟอร์มเข้ามาเอง**
                ⇒ กฎของโปรเจกต์: เลขจากสองแหล่งวางคู่กันได้ **แต่ต้องเขียนว่าต่างกันตรงไหน**
                   ไม่งั้นคนอ่านจะเชื่อว่ามันคือชุดเดียวกัน แล้วสรุปว่าจอพัง */}
            <p className="text-[13px] font-semibold text-gray-700">ยอดขายรายช่องทาง (30 วันล่าสุด)</p>
            <p className="text-[11px] text-gray-400 mb-3 leading-relaxed">
              นับจาก<b>กระจกออเดอร์</b> (ของที่ผ่าน ZORT มา) — <b>คนละกอง</b>กับการ์ด
              {' '}“ท่อดึงออเดอร์ตรงจาก API” ข้างล่าง ที่นับเฉพาะออเดอร์ซึ่งท่อดึงเข้ามาเอง
              {' '}⇒ ช่องทางหนึ่งมียอดตรงนี้ แต่ท่อตรงยัง “ยังไม่เดิน” ได้ตามปกติ
            </p>
            {recentLoading
          ...
          <Card padded={false} className="overflow-hidden">
            <div className="px-4 md:px-5 py-3 border-b border-gray-100">
              ...
              <p className="text-[11px] text-gray-400 mt-0.5">
                นี่คือทางที่ทำให้เราเลิกพึ่ง ZORT ได้ — สถานะข้างล่างคิดจาก<b>ออเดอร์ที่ท่อดึงตรงเข้ามาเอง</b>
                {' '}<b>ไม่นับ</b>ออเดอร์ที่มาทางกระจก ZORT ⇒ ตัวเลขจึงไม่ตรงกับการ์ดยอดขายข้างบน และนั่นถูกต้อง
              </p>
            </div>
            {PIPES.map((p) => {
