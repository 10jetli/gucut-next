...
export default function LedgerScreen({
  ...
  tabs?: string[]
  /** ช่องค้นหาที่ **จอ ZORT ของหน้านั้นมี แต่เรายังไม่มี** พร้อมเหตุผล
   *  🔴 ต้องส่งมาจากจอที่ใช้ เพราะแต่ละหน้าของ ZORT มีช่องไม่เหมือนกัน
   *     (กดอ่านของจริงแล้วเท่านั้น ห้ามเดาจากหน้าอื่น) */
  missingAdv?: { what: string; why: string }[]
  ...
  return (
    <div className="p-4 md:p-6">
      ...
      <AdvancedSearch
        ...
        /* 🔎 ช่องที่ ZORT มีแต่เราไม่มี — จอที่เรียกใช้ส่งมาเอง (ไม่เดาแทนหน้าอื่น)
           ยิงตรวจแล้วว่าเส้น `?zortlist=` รับแค่ keyword/from/to ⇒ ช่องอื่นใส่ไปก็ไม่มีผล */
        notAvailable={missingAdv}
                    ...
                    : zErr
                      ...
                      : zortAnswered
                        ...
                        : cantRead
                          ...
                          /* 🔴 **ต่อท่อแล้วแต่ยังตอบไม่เสร็จ ≠ ยังไม่ได้ต่อท่อ** (เจอบนจอจริง 16 ก.ย. 2569)
                             ระหว่างรอคำตอบ จอเคยขึ้นว่า "ตารางว่างเพราะเรายังไม่ได้ต่อท่อ" ทั้งที่
                             หัวตารางข้าง ๆ กันเขียนว่า "กำลังถาม ZORT…" ⇒ สองประโยคขัดกันในจอเดียว
                             ⇒ จอที่มี `zortList` ต้องพูดว่า **ยังไม่รู้** ไม่ใช่ **ยังไม่ได้ทำ** */
                          : zortList
                            ? <><b>กำลังถาม ZORT อยู่</b> — ยังไม่รู้ว่ามีกี่รายการ · </>
                            : emptyProof
