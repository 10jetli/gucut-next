      ...
      <div className="px-3.5 pb-3 text-[12px] text-gray-700 leading-relaxed">
        ...
        {เต็ม && (
          <>
            ...
            {(n(หัว?.quietDays) !== null || n(หัว?.lookbackDays) !== null || n(หัว?.minSold) !== null) && (
              <p className="text-gray-500 mt-0.5">
                เกณฑ์ของท่อ: เงียบเกิน <b>{fmtNum(n(หัว?.quietDays) ?? 0)}</b> วัน ·
                {' '}ย้อนดู <b>{fmtNum(n(หัว?.lookbackDays) ?? 0)}</b> วัน ·
                {' '}เคยขายอย่างน้อย <b>{fmtNum(n(หัว?.minSold) ?? 0)}</b> ชิ้น
              </p>
            )}
