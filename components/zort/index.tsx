...
/* ── แถบคำสั่งของแถวที่เลือก ───────────────────────────────────────────
   🔴 **ทำไมต้องติดขอบล่าง ไม่ใช่วางเหนือตาราง** (เจอตอนลองใช้ด้วยนิ้วบนแท็บเล็ต 16 ก.ย. 2569)
      แถบที่โผล่เหนือตาราง **ดันทั้งตารางเลื่อนลง** ⇒ ติ๊กใบแรกเสร็จ แถวขยับ
      แล้วนิ้วที่กำลังจะกดใบที่สองไปโดน "แถวอื่น" หรือ "เลือกทั้งหน้า" (เกิดจริงตอนวัด: ติ๊กไป 33 ใบ)
   ⇒ วางไว้ **ท้ายตาราง** แล้วให้ `sticky bottom-0`:
      · โผล่/หายแล้วแถวในตารางไม่ขยับสักพิกเซล
      · เลื่อนดูรายการยาว ๆ แถบยังอยู่กับตาตลอด (คนแพ็กของเลือกไปเลื่อนไป)
   ⚠️ ต้องมีพื้นทึบ ไม่งั้นตัวหนังสือแถวล่างทะลุขึ้นมาอ่านปนกัน */
export function BulkBar({ children }: { children: ReactNode }) {
  return (
    <div className="sticky bottom-0 z-20 mt-2 flex flex-wrap items-center gap-2 text-[12.5px]
      bg-[#eef1fa] border border-[#c9d4f5] rounded px-3 py-2 shadow-[0_-2px_8px_rgba(34,42,87,0.08)]">
      {children}
    </div>
  )
}
 ...
 *
 *  🔴 **ตัวนี้แปลง "คำ" เป็น "สี" เท่านั้น ปลอดภัยข้ามแพลตฟอร์ม — แต่ตัวแปล "คำ" ไม่ปลอดภัย**
 *     (ฝั่งท่อเตือน 6 ก.ย. 2569 ตอนต่อ TikTok)
 *     สถานะ TikTok มีสองภาษา: ผ่าน ZORT เป็นรหัสตัวเลข (121/130/140) ·
 *     ท่อตรงเป็นตัวหนังสือ (COMPLETED/CANCELLED) **ซึ่งสะกดชนกับ Shopee เป๊ะ**
 *     ⇒ ห้ามเอาค่าจากตาราง tiktok_orders ไปแปลด้วย STATUS_TH ของจอรายการขาย
 *       (แผนที่นั้นทำไว้สำหรับค่าดิบของ ZORT: Success/Voided/Pending/Waiting)
 *       แปลข้ามแหล่งเมื่อไหร่ **ยอดจะไปกองผิดแพลตฟอร์มแบบเงียบ ๆ** ไม่มีอะไรฟ้อง
 *     ⇒ ถ้าวันไหนทำจอที่โชว์ค่าจาก tiktok_orders ตรง ๆ **ต้องมีแผนที่ของ TikTok แยกต่างหาก
 *       และรับรหัสตัวเลขด้วย** ไม่ใช่ยืมของเดิม */
 ...
 *    ⇒ ยิงของจริง 5 ก.ย. ได้ `checked: [gucut, shopee, lazada]` · เหลือ TikTok
 *    ✅ **6 ก.ย. 05:00 TikTok เชื่อมสำเร็จ** — ยิงซ้ำได้ `[gucut, shopee, tiktok, lazada]` ครบแล้ว
 *    (บันทึกวันที่ไว้ทั้งสองบรรทัด เพราะประโยคที่ไม่มีวันที่จะกลายเป็นคำโกหกเงียบ ๆ)
...
/** แถบ "กำลังอัปเดต" ของจอที่วาดจากแคชในหน่วยความจำ
 *
 *  ⚠️ **ต้องวางเป็นชิ้นแรกสุดของหน้า เหนือหัวจอ** — กติกาข้อ ④ ที่ตกลงกับฝั่งท่อ:
...
export function MarketCoverage(
   ...
   *  ของจริงตอนเขียน (5 ก.ย.): TikTok ไม่ได้พัง — ยังไม่เคยถูกกดอนุญาต ซึ่งเจ้าของร้านทำได้เลย
 *  ✅ **กดแล้ว 6 ก.ย. 05:00 — ตอนนี้ไม่มีเจ้าไหนค้าง** · ตัวอย่างนี้เก็บไว้เพราะยังอธิบายกฎได้
 *     (ข้อความบนจอไม่ได้เขียนตายตัว มันอ่านจาก failed/notConnected ของท่อ จึงหายเองแล้ว)
