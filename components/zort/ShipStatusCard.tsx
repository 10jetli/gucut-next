...
export default function ShipStatusCard(
  ...
  return (
    <div className="bg-white border border-gray-200 rounded-md px-3.5 py-3 mb-3">
      ...
      <div className="flex gap-2 flex-wrap">
        {shown.map((g) => {
          ...
          const tip = [
            ...
            g.unverified ? 'ท่อแจ้งว่ากองนี้มีรหัสที่ยังไม่ได้ยืนยันกับเอกสารทางการปนอยู่ — ดูค่าดิบข้างบน' : '',
          ].filter(Boolean).join('\n')
          return (
      ...
      {/* กองที่ท่อติดธง unverified = มีรหัสที่ยังไม่ได้เทียบกับคู่มือทางการปนอยู่ — ต้องเขียนบอก
          ไม่ใช่ปล่อยให้ดูเท่ากับกองที่ยืนยันแล้ว
          🔴 **ห้ามเขียนชื่อแพลตฟอร์มตายตัวตรงนี้** (เคยเขียนว่า "TikTok" แล้วต่อท้ายว่า
             "ใบที่มาจาก Shopee กับ Lazada ยืนยันแล้ว" — แก้ 13 ก.ย. 2569)
             วันที่ท่อย้าย Lazada `confirmed` มาไว้กอง done พร้อมธงนี้ (order-status.mjs c20b1d2)
             จอจะประกาศว่า **Lazada ยืนยันแล้ว** ทั้งที่ตัวที่ยังไม่ยืนยันคือ Lazada เอง
             ⇒ ไม่ใช่แค่คำผิด แต่เป็นการ **ล้างมลทินให้เจ้าที่กำลังมีปัญหา**
             เหตุผลเดียวกับกฎบนหัวไฟล์: จอไม่รู้จักรหัสดิบเลยสักตัว ⇒ จออย่าเดาว่าเป็นของใคร
             ของที่บอกได้ว่าเป็นของใครคือ `raws` ซึ่งท่อส่งมาให้แล้ว (ชี้เมาส์ที่กองเพื่อดู) */}
      {anyUnverified && (
        <p className="text-[12px] text-gray-500 mt-2 leading-relaxed">
          ⚠️ กองที่มีจุด <span className="text-amber-600">•</span> คือกองที่ท่อแจ้งว่า
          {' '}<b>มีรหัสที่ยังไม่ได้เทียบกับคู่มือทางการของแพลตฟอร์มปนอยู่</b>
          {' '}(ตรงกับค่าที่พบจริงในข้อมูล แต่ยังไม่ได้ยืนยันกับเอกสารของเขา) —
          {' '}ชี้เมาส์ที่กองนั้นเพื่อดูว่าค่าดิบตัวไหน
        </p>
      )}
    </div>
  )
