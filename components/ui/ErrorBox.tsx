...
import { นับกล่องแดง } from '@/lib/จอพังอยู่'
...
export default function ErrorBox({
  ...
  const เป็นสถานะที่สาม = !!lead && lead.startsWith(SKIP)
  /* 🔑 บอกทะเบียนกลางว่า "ตอนนี้จอนี้มีกล่องแดงอยู่" เพื่อให้แถบป้ายบนหัวจอขึ้นคำเตือน**เหนือป้าย**
     (เหตุผลเต็มอยู่ใน `lib/จอพังอยู่.ts`) · **ไม่นับสถานะที่สาม** เพราะนั่นไม่ใช่ความล้มเหลว */
  useEffect(() => {
    if (เป็นสถานะที่สาม) return
    return นับกล่องแดง()
  }, [เป็นสถานะที่สาม])
  if (เป็นสถานะที่สาม) {
