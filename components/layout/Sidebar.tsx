...
const BAR_BG = '#222a57'        // พื้นแถบทั้งแถบ — **วัดจากพิกเซลภาพจริง** (พบ 73%)
const GROUP_LINE = '#2f6fe0'    // เส้นสว่างคาดใต้หัวข้อกลุ่มที่กางอยู่
const CHILD_ACTIVE = '#4669e5'  // ลูกที่เลือกอยู่ — **วัดจากพิกเซลภาพจริง** (พบ 90% ในแถบที่เลือก)
const ITEM_ACTIVE = '#4669e5'   // เมนูเดี่ยวที่เลือกอยู่ — ZORT ใช้สีเดียวกับลูกที่เลือก
    ...
    >
      ...
      {/* การ์ดสถานะมุมล่างซ้าย — ZORT มีอันนี้ติดอยู่ทุกจอ (ภาพครอปจากเจ้าของร้าน)
          ⚠️ ของ ZORT เช็คเองอัตโนมัติ **แต่ของเราห้าม** — เจ้าของร้านสั่งไว้ว่า
             หน้าสถานะระบบต้องกดเช็คเอง (ประหยัดทรัพยากร · ตัวตรวจยิงของจริงหลายสิบเรื่อง)
             ⚠️ **ห้ามเขียนจำนวนเรื่องเป็นตัวเลขตรงนี้** — เลขนี้โตทุกครั้งที่ฝั่งท่อเพิ่มตัวตรวจ
                (เคยเขียน 24 · ของจริง 6 ก.ย. 2569 = 33) · กติกา 'ใส่วันที่กำกับ' ไม่พอกับเลขคลาสนี้
                เพราะมันโตเองโดยไม่มีใครแตะจอ ⇒ **เลขที่โตเองห้ามเขียนเป็นตัวเลข**
      ...
      {!collapsed && (
        <div className="p-2 shrink-0">
          ...
          >
            ...
            {/* 🔴 **ห้ามเขียนจำนวนเรื่องที่ตรวจตายตัวตรงนี้** (แก้ 6 ก.ย. 2569)
                เดิมเขียน "24 เรื่อง" · ของจริงตอนนี้ 33 (นับ `check(` ใน status.mjs ฝั่งท่อ)
                เลขนี้เพิ่มทุกครั้งที่ฝั่งท่อเพิ่มตัวตรวจ **โดยไม่มีใครมาแก้ตรงนี้**
                ⇒ ตัวเลขที่คนอ่านเห็นจะต่ำกว่าความจริงลงเรื่อย ๆ และไม่มีอะไรฟ้อง
                ⚠️ CLAUDE.md เตือนเรื่องนี้ไว้ตรง ๆ แล้วว่าเคยค้างที่ 24 ทั้งที่ของจริง 30
                   ⇒ ตัดตัวเลขทิ้ง ดีกว่าใส่เลขที่ต้องมีคนคอยตามแก้
                   (จอสถานะระบบนับเองจากผลจริงอยู่แล้วตอนกดเข้าไป) */}
