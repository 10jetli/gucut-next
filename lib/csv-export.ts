 ...
 *     ⚠️ ถ้าวนไม่ครบ (ชนเพดานกันวน / ท่อล่มกลางคัน) **ห้ามให้ไฟล์ออกเลย** — ดู `exportBlocked()`
 *        (ท่านประธานตัดสินเอง 18 ก.ย. 2569 · ของเดิมปล่อยไฟล์แล้วเขียนเตือนไว้หัวไฟล์
 *         ซึ่งคำเตือนหายทันทีที่คน copy เฉพาะตารางไปวางต่อ)
...
/**
 * 🔴 **ด่านห้ามให้ไฟล์ออก — ท่านประธานสั่งเอง 18 ก.ย. 2569 (ตอนอนุมัติให้ส่งออกได้ทุกจอ)**
 *
 * คำสั่งเต็ม: *"ถ้าไล่ไม่ครบเพราะยิงพลาดกลางทาง **ห้ามให้ไฟล์ออก** บอกว่าไม่สำเร็จแทน"*
 *
 * ⚠️ **ของเดิมทำตรงข้ามเป๊ะ** — ได้ไม่ครบก็ยังดาวน์โหลด แล้วเขียนเตือนไว้บรรทัดแรกของไฟล์
 *    เหตุผลเดิม (เขียนไว้หัวไฟล์นี้ 15 ก.ย.) คือ "ไฟล์ที่ขาดแถวเงียบ ๆ อันตรายกว่าไฟล์ที่ไม่มี"
 *    ⇒ **ยังจริงอยู่ แต่ไม่ใช่ทางเลือกเดียว** · ท่านเลือกอีกทาง: ไม่ให้ไฟล์เลย
 *    เพราะบรรทัดเตือนในไฟล์ **รอดได้แค่ถึงคนแรก** — พอเปิดใน Excel แล้ว copy
 *    เฉพาะตารางไปวางต่อ คำเตือนจะหายไปกับมือโดยที่ตัวเลขยังอยู่ครบ
 *    (คลาสเดียวกับ "ระบบทำงานถูก แต่สื่อสารผิด" ใน CLAUDE.md — จอ/ไฟล์ดูปกติทุกประการ)
 *
 * คืน `null` = ปล่อยไฟล์ได้ · คืนข้อความ = **ห้ามดาวน์โหลด** แล้วเอาข้อความนี้ขึ้นจอ
 *
 * ⚠️ **ถ้อยคำต้องจริงกับทุกจอที่เรียกใช้** — ร่างแรกเขียนว่า "หั่นช่วงวันให้แคบลง"
 *    แต่จอหมวดหมู่ · สินค้าเป็นชุด · คลังสินค้า **ไม่มีช่องวันเลย** ⇒ คำแนะนำนั้นทำตามไม่ได้
 *    (ด่าน check-inherited เป็นคนทวงให้อ่าน ⇒ เปลี่ยนเป็น "กรองให้แคบลง" ซึ่งจริงทุกจอ)
 *
 * ⚠️ "ท่อไม่ได้บอกจำนวนทั้งหมด" (`total === null`) **ไม่ใช่เหตุห้าม** — นั่นคือ "ยังไม่รู้ว่าครบไหม"
 *    ไม่ใช่ "รู้ว่าไม่ครบ" · ห้ามเอาสองอย่างนี้มารวมกัน (กฎสามสถานะของโปรเจกต์)
 *    ⇒ กรณีนั้นไฟล์ออกได้ แต่หัวไฟล์ต้องเขียนตรง ๆ ว่ายังไม่รู้ (coverageText ทำให้แล้ว)
 */
export function exportBlocked(c: Coverage): string | null {
  if (c.stoppedBecause) {
    return `ยังไม่ให้ไฟล์ออก เพราะไล่ข้อมูลไม่ครบ — หยุดเพราะ ${c.stoppedBecause}`
      + ` (ได้มา ${c.got.toLocaleString('th-TH')} แถว`
      + (c.total === null ? '' : ` จาก ${c.total.toLocaleString('th-TH')} แถว`)
      + ') · ลองใหม่อีกครั้ง หรือกรองให้แคบลงแล้วส่งออกทีละชุด'
  }
  if (c.problems?.length) {
    return `ยังไม่ให้ไฟล์ออก เพราะข้อมูลรอบนี้ไม่สมบูรณ์ — ${c.problems.join(' · ')}`
      + ' · ไฟล์ที่ขาดแถวจะถูกเอาไปเทียบยอด แล้วไม่มีอะไรฟ้องว่ามันขาด'
  }
  if (c.total !== null && c.got < c.total) {
    return `ยังไม่ให้ไฟล์ออก เพราะได้ ${c.got.toLocaleString('th-TH')} แถว`
      + ` จากทั้งหมด ${c.total.toLocaleString('th-TH')} แถว — ยังไม่ครบ`
      + ' · ลองใหม่อีกครั้ง หรือกรองให้แคบลงแล้วส่งออกทีละชุด'
  }
  return null
}
...
/**
 * ชื่อไฟล์ตามกติกาข้อ ③ ของท่านประธาน: **ต้องมีวันที่และขอบเขตอยู่ในชื่อ**
 * เหตุผลที่ท่านให้: ไฟล์ออกจากระบบไปแล้วจะไปนอนรวมกันในเครื่องคน
 * ⇒ ชื่อคือสิ่งเดียวที่บอกได้ว่าอันไหนใหม่กว่า และอันไหนกรองอะไรไว้
 *
 * ⚠️ วันที่นี่คือ **วันที่ดึงข้อมูล** ไม่ใช่ช่วงวันของข้อมูล — จึงเขียนคำว่า "ดึง" กำกับเสมอ
 *    (ชื่อที่มีแต่ตัวเลขวันที่ ทำให้คนเดาว่าเป็นช่วงข้อมูล แล้วเทียบผิดใบ)
 */
export function exportFilename(base: string, scope: string | undefined, now = new Date()): string {
  const th = new Date(now.getTime() + 7 * 3600_000)
  const p = (n: number) => String(n).padStart(2, '0')
  const วัน = `${p(th.getUTCDate())}-${p(th.getUTCMonth() + 1)}-${th.getUTCFullYear() + 543}`
  /* ตัดอักขระที่ตั้งชื่อไฟล์ไม่ได้บนวินโดวส์ออก — ชื่อไทยใช้ได้ แต่ / \ : * ? " < > | ใช้ไม่ได้ */
  const safe = (s: string) => s.replace(/[\\/:*?"<>|]+/g, ' ').replace(/\s+/g, ' ').trim()
  const ขอบเขต = scope ? `-${safe(scope)}` : ''
  return `${safe(base)}${ขอบเขต}-ดึง${วัน}`
}
