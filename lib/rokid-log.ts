...
export interface RokidLogEntry {
  ...
  stream: boolean
  /* 🔔 **สัญญาณชีพ — ต้องเป็นฟิลด์จริง ห้ามให้จอไปอ่านจากข้อความ**
     ครั้งแรกผมยัดไว้ใน `question` เป็นข้อความไทย ⇒ จอที่ไม่รู้จักจะวาดเป็น
     การ์ดบทสนทนาที่ช่องคำถามว่าง แล้วเขียนว่า "(ถอดเสียงไม่ได้ข้อความ)"
     ⇒ **สัญญาณที่รอมาสองวัน จะหน้าตาเหมือนไมค์พัง** และเราอาจสรุปว่าไม่มา ทั้งที่มาแล้ว
     (ฝั่งจอจับได้ก่อนเจ้าของร้านกดทดสอบ · เป็นกับดักเดียวกับที่ไล่แก้กันทั้งวัน)
     🔑 กฎที่ละเมิดไป: **ฟิลด์ที่เป็นคำอธิบาย ห้ามมีอำนาจตัดสินใจ**
        จะให้จอตัดสินใจ ต้องมีฟิลด์ความหมายเดียวให้มันอ่าน */
  ping?: boolean
  /** รุ่นของโค้ดที่เครื่องรันอยู่จริง — ตอบ "อุปกรณ์ดึงเวอร์ชันไหนไป" ได้ด้วยตัวเอง */
  build?: string
}
