...
export interface StaffClaims {
  ...
  exp: number
  /** 🔴 **ชั้นสิทธิ์ — ไม่มีช่องนี้ = พนักงาน** (เพิ่ม 14 ก.ย. 2569 · งานกระดาน t_mu1bkqy4)
   *
   *  ⚠️ **ต้องเข้ากันได้ย้อนหลัง** — โทเคนที่ออกไปแล้วไม่มีช่องนี้ และยังไม่หมดอายุอีก 8 ชม.
   *     ⇒ ไม่มี `r` ต้องแปลว่า "พนักงาน" เหมือนเดิมเป๊ะ **ห้ามตีเป็นสิทธิ์ที่สูงกว่า**
   *     (ตีผิดข้างนี้ = คนที่ล็อกอินค้างอยู่ได้สิทธิ์เพิ่มขึ้นเองโดยไม่มีใครกด)
   *  ⚠️ ค่าที่ไม่รู้จักก็ต้องตกเป็น "พนักงาน" ด้วย — ห้ามเชื่อค่าในโทเคนแบบเปิดกว้าง */
  r?: StaffRole
}

/** ชั้นสิทธิ์ของผู้ใช้ที่เพิ่มจากหน้าเว็บ · แอดมินไม่ได้อยู่ในชุดนี้ (ใช้ SITE_PASSWORD คนละทาง) */
...
export const roleOf = (claims: StaffClaims | null): StaffRole =>
  claims?.r === 'account' ? 'account' : 'staff'
