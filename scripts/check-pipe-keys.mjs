  ...
  for (const [ชื่อ, เนื้อ] of อ่านinterface(src)) {
    for (const [, ช่อง] of ตัดคอมเมนต์(เนื้อ).matchAll(fieldRe)) {
      ...
      /* 🔴 **คีย์ที่ท่อประกอบชื่อตอนรันไม่มีอยู่ในซอร์สเป็นข้อความ** (เจอ 19 ก.ย. 2569)
         ...
         ⇒ ค้นหาชื่อเต็ม `recipeExpectedEveryHours` ในซอร์สจะ **ไม่เจอตลอดกาล**
         ⇒ ด่านนี้ฟ้องจอที่อ่านคีย์ที่ท่อส่งมาจริง ๆ = ธงลวงที่สั่งให้คนไปแก้ของที่ถูก
         🔑 คลาสเดียวกับที่ฝั่งท่อเจอวันนี้ (grep เจอชื่อตัวแปรภายใน ไม่ใช่ชื่อที่วิ่งบนสาย)
            แต่กลับด้าน: **grep มองไม่เห็นชื่อที่ถูกประกอบขึ้นตอนรัน**
         ⇒ ถ้าท่อมีเทมเพลตที่ลงท้ายด้วยส่วนท้ายเดียวกัน ให้ถือว่ารู้จักคีย์นั้น
            (ยังแคบพอ เพราะต้องเจอเทมเพลตจริงในซอร์ส ไม่ใช่เดาเอง) */
      const ส่วนท้าย = ช่อง.match(/^[a-z]+([A-Z]\w+)$/)?.[1]
      if (ส่วนท้าย && new RegExp('\\$\\{[^}]*\\}' + ส่วนท้าย + '\\b').test(ข้อความท่อ)) continue
      พบ.push(`${rel}  ${คีย์}`)
    }
  }
}
