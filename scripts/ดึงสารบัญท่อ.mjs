...
/* 🔑 **เก็บ `ตัวกรองที่รับ` รายเส้นด้วย** (เพิ่ม 19 ก.ย. 2569 หลังฝั่งท่อประกาศครบ 24 เส้น)
   ...
   ⚠️ ขอบเขตที่ฝั่งท่อกำชับ: ใช้ยืนยัน **ทิศบวก** เท่านั้น — "เส้นนี้รับตัวนี้จริง"
      🚫 ห้ามใช้ตัดสินว่า "ไม่รับ" เพราะ 5 เส้นส่ง `null` (ยังประเมินไม่ได้)
         และการไม่อยู่ในรายการ ไม่เท่ากับยืนยันว่าเมิน */
async function ตัวกรองรายเส้น(lists) {
  ...
  for (const ชื่อ of lists) {
    ...
    out[ชื่อ] = สัญญา && "ตัวกรองที่รับ" in สัญญา
      ? { มีคีย์: true, ค่า: สัญญา["ตัวกรองที่รับ"] ?? null, ที่มา: สัญญา["ตัวกรองที่รับ—ที่มา"] ?? null }
      : { มีคีย์: false, ค่า: null, ที่มา: null };
  }
  return out;
}
