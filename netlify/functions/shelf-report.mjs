...
//
// 🔴 **เปิดส่งอัตโนมัติแล้วตั้งแต่ 12 ก.ย. 2569** — `export const config` ท้ายไฟล์
//    ตั้ง `schedule: "0 0 * * *"` (UTC) = **ตี 7 ตามเวลาไทย** · รอบที่ Netlify ยิงเองจะ `send` เสมอ
//    ⇒ **ของจริงเข้ากลุ่ม Telegram ของท่านประธานทุกเช้า** ไม่ใช่โหมดซ้อมอีกต่อไป
//    ⚠️ ข้อความที่เข้ากลุ่มแล้ว **เอาคืนไม่ได้** และท่านประธานอ่านทุกใบ (เหตุผลเดิมจาก CEO)
//       ⇒ แก้ถ้อยคำในไฟล์นี้ = แก้ของที่ส่งถึงท่านประธานพรุ่งนี้เช้า · ซ้อมด้วยการ **ไม่ใส่** `?send=1`
//    🔴 คอมเมนต์ตรงนี้เคยเขียนว่า "ยังไม่เปิดส่ง ไม่มี config.schedule" **ค้างอยู่ 1 วันหลังเปิดจริง**
//       (แก้ 13 ก.ย. 2569) — คำอธิบายที่บอกว่า "ยังไม่ส่ง" บนของที่ส่งอยู่ ทำให้คนถัดไป
//       ทดลองแบบไม่ระวังแล้วข้อความจริงเข้ากลุ่ม · คอมเมนต์ผิดบนเส้นที่ย้อนกลับไม่ได้ = บั๊ก
//
